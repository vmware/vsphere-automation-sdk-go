...
func (dIface *DataClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(dIface.interfaceName)
}

func (dIface *DataClientImpl) Definition() core.InterfaceDefinition {
    return dIface.interfaceDefinition
}

func (dIface *DataClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (dIface *DataClientImpl) QueryDataPoints(filterParam *FilterSpec) (DataPointsResult, error) {
	typeConverter := dIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(dIface.interfaceIdentifier, "query_data_points")
	sv := bindings.NewStructValueBuilder(queryDataPointsInputType(), typeConverter)
	sv.AddStructField("Filter", filterParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput DataPointsResult
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := queryDataPointsRestMetadata()
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), queryDataPointsOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(DataPointsResult), nil
	} else {
...
func (dIface *DataClientImpl) queryDataPointsMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(queryDataPointsInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(queryDataPointsOutputType())
      if(inputError != nil) {
