...
func (cIface *CounterMetadataClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(cIface.interfaceName)
}

func (cIface *CounterMetadataClientImpl) Definition() core.InterfaceDefinition {
    return cIface.interfaceDefinition
}

func (cIface *CounterMetadataClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (cIface *CounterMetadataClientImpl) List(cidParam string, filterParam *FilterSpec) ([]Info, error) {
...
func (cIface *CounterMetadataClientImpl) GetDefault(cidParam string) ([]Info, error) {
	typeConverter := cIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(cIface.interfaceIdentifier, "get_default")
	sv := bindings.NewStructValueBuilder(getDefaultInputType(), typeConverter)
	sv.AddStructField("Cid", cidParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput []Info
	...
	operationRestMetaData := getDefaultRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= cIface.Invoke(cIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput []Info
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), getDefaultOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.([]Info), nil
...
func (cIface *CounterMetadataClientImpl) Get(cidParam string, midParam string) (Info, error) {
	typeConverter := cIface.connector.TypeConverter()
...
func (cIface *CounterMetadataClientImpl) getDefaultMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(getDefaultInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(getDefaultOutputType())
      if(inputError != nil) {
