...
func (hIface *HealthClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(hIface.interfaceName)
}

func (hIface *HealthClientImpl) Definition() core.InterfaceDefinition {
    return hIface.interfaceDefinition
}

func (hIface *HealthClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (hIface *HealthClientImpl) Messages(itemParam string) ([]appliance.Notification, error) {
	typeConverter := hIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(hIface.interfaceIdentifier, "messages")
	sv := bindings.NewStructValueBuilder(messagesInputType(), typeConverter)
	sv.AddStructField("Item", itemParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput []appliance.Notification
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := messagesRestMetadata()
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), messagesOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.([]appliance.Notification), nil
	} else {
...
func (hIface *HealthClientImpl) messagesMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(messagesInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(messagesOutputType())
      if(inputError != nil) {
