...
func (pIface *ProxyClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(pIface.interfaceName)
}

func (pIface *ProxyClientImpl) Definition() core.InterfaceDefinition {
    return pIface.interfaceDefinition
}

func (pIface *ProxyClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (pIface *ProxyClientImpl) Test(hostParam string, protocolParam string, configParam Config) (TestResult, error) {
...
func (pIface *ProxyClientImpl) Set(protocolParam string, configParam Config) error {
	typeConverter := pIface.connector.TypeConverter()
...
func (pIface *ProxyClientImpl) List() (map[Protocol]Config, error) {
	typeConverter := pIface.connector.TypeConverter()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput map[Protocol]Config
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
    ...
    if methodResult.IsSuccess() {
	    ...
	    return output.(map[Protocol]Config), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), pIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (pIface *ProxyClientImpl) Get(protocolParam string) (Config, error) {
	typeConverter := pIface.connector.TypeConverter()
