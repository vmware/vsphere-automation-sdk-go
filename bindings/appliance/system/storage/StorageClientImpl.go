...
func (sIface *StorageClientImpl) List() ([]StorageMapping, error) {
	...
	if inputError != nil {
        var emptyOutput []StorageMapping
    ...
    if methodResult.IsSuccess() {
	    ...
	    return output.([]StorageMapping), nil
	} else {
...
func (sIface *StorageClientImpl) Resize() error {
	...
	sv := bindings.NewStructValueBuilder(resizeInputType(), typeConverter)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		return bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := resizeRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= sIface.Invoke(sIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
    if methodResult.IsSuccess() {
...
func (sIface *StorageClientImpl) ResizeEx() (map[string]StorageChange, error) {
	typeConverter := sIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(sIface.interfaceIdentifier, "resize_ex")
	sv := bindings.NewStructValueBuilder(resizeExInputType(), typeConverter)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput map[string]StorageChange
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := resizeExRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= sIface.Invoke(sIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput map[string]StorageChange
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), resizeExOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(map[string]StorageChange), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (sIface *StorageClientImpl) resizeMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(resizeInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(resizeOutputType())
      if(inputError != nil) {
...
func (sIface *StorageClientImpl) resizeExMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(resizeExInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(resizeExOutputType())
      if(inputError != nil) {
