...
func (tIface *TlsClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(tIface.interfaceName)
}

func (tIface *TlsClientImpl) Definition() core.InterfaceDefinition {
    return tIface.interfaceDefinition
}

func (tIface *TlsClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (tIface *TlsClientImpl) Set(specParam Spec) error {
...
func (tIface *TlsClientImpl) Get() (Info, error) {
	typeConverter := tIface.connector.TypeConverter()
...
func (tIface *TlsClientImpl) Renew(durationParam *int64) error {
	...
	sv := bindings.NewStructValueBuilder(renewInputType(), typeConverter)
	sv.AddStructField("Duration", durationParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		return bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := renewRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= tIface.Invoke(tIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
    if methodResult.IsSuccess() {
...
func (tIface *TlsClientImpl) ReplaceVmcaSigned(specParam ReplaceSpec) error {
	typeConverter := tIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(tIface.interfaceIdentifier, "replace_vmca_signed")
	sv := bindings.NewStructValueBuilder(replaceVmcaSignedInputType(), typeConverter)
	sv.AddStructField("Spec", specParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		return bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := replaceVmcaSignedRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= tIface.Invoke(tIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
    if methodResult.IsSuccess() {
...
func (tIface *TlsClientImpl) renewMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(renewInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(renewOutputType())
      if(inputError != nil) {
...
func (tIface *TlsClientImpl) replaceVmcaSignedMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(replaceVmcaSignedInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(replaceVmcaSignedOutputType())
      if(inputError != nil) {
