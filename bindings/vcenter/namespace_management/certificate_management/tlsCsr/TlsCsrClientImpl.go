...
func (tIface *TlsCsrClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(tIface.interfaceName)
}

func (tIface *TlsCsrClientImpl) Definition() core.InterfaceDefinition {
    return tIface.interfaceDefinition
}

func (tIface *TlsCsrClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (tIface *TlsCsrClientImpl) Create(clusterParam string, specParam Spec) (string, error) {
...
func (tIface *TlsCsrClientImpl) CreateNcpDefaultIngressTls(clusterParam string, specParam Spec) (string, error) {
	typeConverter := tIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(tIface.interfaceIdentifier, "create_ncp_default_ingress_tls")
	sv := bindings.NewStructValueBuilder(createNcpDefaultIngressTlsInputType(), typeConverter)
	sv.AddStructField("Cluster", clusterParam)
	sv.AddStructField("Spec", specParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
	...
	operationRestMetaData := createNcpDefaultIngressTlsRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= tIface.Invoke(tIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput string
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), createNcpDefaultIngressTlsOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(string), nil
...
func (tIface *TlsCsrClientImpl) createNcpDefaultIngressTlsMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(createNcpDefaultIngressTlsInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(createNcpDefaultIngressTlsOutputType())
      if(inputError != nil) {
