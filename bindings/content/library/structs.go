...
// Specifies the properties that can be used as a filter to find libraries. When multiple properties are specified, all properties of the library must match the specification.
 type FindSpec struct {
    Name *string
    Type_ *content.LibraryModel_LibraryType
}
...
func FindSpecBindingType() bindings.BindingType {
    ...
    fields["type"] = bindings.NewOptionalType(bindings.NewEnumType("com.vmware.content.library_model.library_type", reflect.TypeOf(content.LibraryModel_LibraryType(content.LibraryModel_LibraryType_LOCAL))))
    fieldNameMap["type"] = "Type_"
    var validators = []bindings.Validator{}
    return bindings.NewStructType("com.vmware.content.library.find_spec",fields, reflect.TypeOf(FindSpec{}), fieldNameMap, validators)
}
