...
type SubscribedItemClient interface {
    ...
    //
    //  The default behavior of the synchronization is determined by the library.SubscriptionInfo of the library which owns the library item. 
    //
    // * If library.SubscriptionInfo#onDemand is true, then the file content is not synchronized by default. In this case, only the library item metadata is synchronized. The file content may still be forcefully synchronized by passing true for the ``force_sync_content`` parameter.
    // * If library.SubscriptionInfo#onDemand is false, then this call will always synchronize the file content. The ``force_sync_content`` parameter is ignored when the subscription is not on-demand.
    //
    //  When the file content has been synchronized, the library.ItemModel#cached property will be true. 
    //
