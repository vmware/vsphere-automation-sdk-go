...
type DownloadSessionClient interface {
    ...
    //
    // * The resource ``com.vmware.content.library.Item`` referenced by the property item.DownloadSessionModel#libraryItemId requires ``ContentLibrary.DownloadSession``.
    Create(clientTokenParam *string, createSpecParam item.DownloadSessionModel) (string, error) 
    ...
    // @return The item.DownloadSessionModel instance with the given ``download_session_id``.
    ...
    Get(downloadSessionIdParam string) (item.DownloadSessionModel, error) 
    ...
    // Keeps a download session alive. This operation is allowed only if the session is in the item.DownloadSessionModel_State#State_ACTIVE state. 
    //
    ...
    // The parameter must be an identifier for the resource type: ``com.vmware.content.library.item.DownloadSession``.
    // @param progressParam  Optional update to the progress property of the session. If specified, the new progress should be greater then the current progress. See item.DownloadSessionModel#clientProgress.
    // If not specified the progress is not updated.
    // @throws NotFound  if no download session with the given identifier exists.
    // @throws NotAllowedInCurrentState  if the download session is not in the item.DownloadSessionModel_State#State_ACTIVE state.
    // @throws com.vmware.vapi.std.errors.Unauthorized if you do not have all of the privileges described as follows: 
    //
    // * Method execution requires ``System.Anonymous``.
