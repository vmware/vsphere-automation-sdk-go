...
func (fIface *FileClientImpl) List(downloadSessionIdParam string) ([]Info, error) {
...
func (fIface *FileClientImpl) Prepare(downloadSessionIdParam string, fileNameParam string, endpointTypeParam *EndpointType) (Info, error) {
	typeConverter := fIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(fIface.interfaceIdentifier, "prepare")
	sv := bindings.NewStructValueBuilder(prepareInputType(), typeConverter)
	sv.AddStructField("DownloadSessionId", downloadSessionIdParam)
	...
	operationRestMetaData := prepareRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	fIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= fIface.Invoke(fIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput Info
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), prepareOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(Info), nil
...
func (fIface *FileClientImpl) Get(downloadSessionIdParam string, fileNameParam string) (Info, error) {
	typeConverter := fIface.connector.TypeConverter()
...
func (fIface *FileClientImpl) prepareMethodDefinition() *core.MethodDefinition {
      interfaceIdentifier := core.NewInterfaceIdentifier(fIface.interfaceName)
      typeConverter := fIface.connector.TypeConverter()

      input, inputError := typeConverter.ConvertToDataDefinition(prepareInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(prepareOutputType())
      if(inputError != nil) {
          log.Errorf("Error in ConvertToDataDefinition for FileClientImpl.prepare method's input - %s",
              bindings.VAPIerrorsToError(inputError).Error())
          return nil
      }
      if(outputError != nil) {
          log.Errorf("Error in ConvertToDataDefinition for FileClientImpl.prepare method's output - %s",
              bindings.VAPIerrorsToError(outputError).Error())
          return nil
      }
	  ...
	  errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.NotFoundBindingType())
	  if(errError1 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for FileClientImpl.prepare method's errors.NotFound error - %s",
	        bindings.VAPIerrorsToError(errError1).Error())
        return nil
	  }
	  errorDefinitions = append(errorDefinitions, errDef1.(data.ErrorDefinition))
	  ...
	  if(errError2 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for FileClientImpl.prepare method's errors.InvalidArgument error - %s",
	  ...
	  if(errError3 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for FileClientImpl.prepare method's errors.Unauthorized error - %s",
