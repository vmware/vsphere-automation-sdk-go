...
type NsProfilesClient interface {
	...
	// Deprecated: This API element is deprecated.
	//
	// @param attributeTypeParam It fetches NSProfiles for the given attribute type. Only one type of supported attribute type can be mentioned in a single API call.API will return all NSProfiles that have at least one attribute level key which matches given attribute type. If not provided, all NSProfiles will be returned. (optional)
