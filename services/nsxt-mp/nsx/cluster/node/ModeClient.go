...
type ModeClient interface {
	...
	Get(clusterNodeIdParam string) (nsxModel.NodeMode, error)
}
...
func NewModeClient(connector vapiProtocolClient_.Connector) *modeClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.mode")
...
func (mIface *modeClient) Get(clusterNodeIdParam string) (nsxModel.NodeMode, error) {
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.mode", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeMode
