...
type CentralConfigClient interface {
	...
	Get(clusterNodeIdParam string) (nsxModel.CentralConfigProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(clusterNodeIdParam string, centralConfigPropertiesParam nsxModel.CentralConfigProperties) (nsxModel.CentralConfigProperties, error)
}
...
func NewCentralConfigClient(connector vapiProtocolClient_.Connector) *centralConfigClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.central_config")
...
func (cIface *centralConfigClient) Get(clusterNodeIdParam string) (nsxModel.CentralConfigProperties, error) {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.central_config", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.CentralConfigProperties
...
func (cIface *centralConfigClient) Update(clusterNodeIdParam string, centralConfigPropertiesParam nsxModel.CentralConfigProperties) (nsxModel.CentralConfigProperties, error) {
	typeConverter := cIface.connector.TypeConverter()
	executionContext := cIface.connector.NewExecutionContext()
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.central_config", "update", inputDataValue, executionContext)
	var emptyOutput nsxModel.CentralConfigProperties
	if methodResult.IsSuccess() {
