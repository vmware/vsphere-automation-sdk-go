...
type MpaConfigClient interface {
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Get(clusterNodeIdParam string) (nsxModel.MPAConfigProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(clusterNodeIdParam string, mPAConfigPropertiesParam nsxModel.MPAConfigProperties) (nsxModel.MPAConfigProperties, error)
}
...
func NewMpaConfigClient(connector vapiProtocolClient_.Connector) *mpaConfigClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.mpa_config")
...
func (mIface *mpaConfigClient) Delete(clusterNodeIdParam string) error {
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.mpa_config", "delete", inputDataValue, executionContext)
...
func (mIface *mpaConfigClient) Get(clusterNodeIdParam string) (nsxModel.MPAConfigProperties, error) {
	typeConverter := mIface.connector.TypeConverter()
	executionContext := mIface.connector.NewExecutionContext()
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.mpa_config", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.MPAConfigProperties
	if methodResult.IsSuccess() {
...
func (mIface *mpaConfigClient) Update(clusterNodeIdParam string, mPAConfigPropertiesParam nsxModel.MPAConfigProperties) (nsxModel.MPAConfigProperties, error) {
	typeConverter := mIface.connector.TypeConverter()
	executionContext := mIface.connector.NewExecutionContext()
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.mpa_config", "update", inputDataValue, executionContext)
	var emptyOutput nsxModel.MPAConfigProperties
	if methodResult.IsSuccess() {
