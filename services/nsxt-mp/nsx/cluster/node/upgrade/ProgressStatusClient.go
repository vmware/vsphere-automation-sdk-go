...
type ProgressStatusClient interface {
	...
	Get(clusterNodeIdParam string) (nsxModel.UpgradeProgressStatus, error)
}
...
func NewProgressStatusClient(connector vapiProtocolClient_.Connector) *progressStatusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.upgrade.progress_status")
...
func (pIface *progressStatusClient) Get(clusterNodeIdParam string) (nsxModel.UpgradeProgressStatus, error) {
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.upgrade.progress_status", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.UpgradeProgressStatus
