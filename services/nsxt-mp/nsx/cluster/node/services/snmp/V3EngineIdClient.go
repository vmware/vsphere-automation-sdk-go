...
type V3EngineIdClient interface {
	...
	Get(clusterNodeIdParam string) (nsxModel.NodeSnmpV3EngineID, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(clusterNodeIdParam string, nodeSnmpV3EngineIDParam nsxModel.NodeSnmpV3EngineID) (nsxModel.NodeSnmpV3EngineID, error)
}
...
func NewV3EngineIdClient(connector vapiProtocolClient_.Connector) *v3EngineIdClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.services.snmp.v3_engine_id")
...
func (vIface *v3EngineIdClient) Get(clusterNodeIdParam string) (nsxModel.NodeSnmpV3EngineID, error) {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.services.snmp.v3_engine_id", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeSnmpV3EngineID
...
func (vIface *v3EngineIdClient) Update(clusterNodeIdParam string, nodeSnmpV3EngineIDParam nsxModel.NodeSnmpV3EngineID) (nsxModel.NodeSnmpV3EngineID, error) {
	typeConverter := vIface.connector.TypeConverter()
	executionContext := vIface.connector.NewExecutionContext()
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.services.snmp.v3_engine_id", "update", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeSnmpV3EngineID
	if methodResult.IsSuccess() {
