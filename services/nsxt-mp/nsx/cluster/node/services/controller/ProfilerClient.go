...
type ProfilerClient interface {
	...
	Get(clusterNodeIdParam string) (nsxModel.ControllerProfilerProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(clusterNodeIdParam string, controllerProfilerPropertiesParam nsxModel.ControllerProfilerProperties) error
}
...
func NewProfilerClient(connector vapiProtocolClient_.Connector) *profilerClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.services.controller.profiler")
...
func (pIface *profilerClient) Get(clusterNodeIdParam string) (nsxModel.ControllerProfilerProperties, error) {
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.services.controller.profiler", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.ControllerProfilerProperties
...
func (pIface *profilerClient) Update(clusterNodeIdParam string, controllerProfilerPropertiesParam nsxModel.ControllerProfilerProperties) error {
	typeConverter := pIface.connector.TypeConverter()
	executionContext := pIface.connector.NewExecutionContext()
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.services.controller.profiler", "update", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), pIface.GetErrorBindingType(methodResult.Error().Name()))
