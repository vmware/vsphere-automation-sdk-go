...
type StatusClient interface {
	...
	// GET https://<nsx-mgr>/api/v1/node/services/nsxt-mp/monitoring/status
	//
	...
	Get(clusterNodeIdParam string) (nsxModel.NodeServiceStatusProperties, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.services.telemetry.status")
...
func (sIface *statusClient) Get(clusterNodeIdParam string) (nsxModel.NodeServiceStatusProperties, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.services.telemetry.status", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeServiceStatusProperties
