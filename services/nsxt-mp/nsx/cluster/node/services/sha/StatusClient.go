...
type StatusClient interface {
	...
	// Returns System Health Agent(SHA) service status
	//
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.services.sha.status")
...
func (sIface *statusClient) Get(clusterNodeIdParam string) (nsxModel.NodeServiceStatusProperties, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.services.sha.status", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeServiceStatusProperties
