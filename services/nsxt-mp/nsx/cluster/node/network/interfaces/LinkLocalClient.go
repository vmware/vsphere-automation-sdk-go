...
type LinkLocalClient interface {
	...
	Create(clusterNodeIdParam string, nodeLinkLocalNetworkInterfaceParam nsxModel.NodeLinkLocalNetworkInterface) (nsxModel.NodeLinkLocalNetworkInterface, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Get(clusterNodeIdParam string) (nsxModel.NodeLinkLocalNetworkInterface, error)
}
...
func NewLinkLocalClient(connector vapiProtocolClient_.Connector) *linkLocalClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.network.interfaces.link_local")
...
func (lIface *linkLocalClient) Create(clusterNodeIdParam string, nodeLinkLocalNetworkInterfaceParam nsxModel.NodeLinkLocalNetworkInterface) (nsxModel.NodeLinkLocalNetworkInterface, error) {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.network.interfaces.link_local", "create", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeLinkLocalNetworkInterface
...
func (lIface *linkLocalClient) Delete(clusterNodeIdParam string) error {
	typeConverter := lIface.connector.TypeConverter()
	executionContext := lIface.connector.NewExecutionContext()
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.network.interfaces.link_local", "delete", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), lIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (lIface *linkLocalClient) Get(clusterNodeIdParam string) (nsxModel.NodeLinkLocalNetworkInterface, error) {
	typeConverter := lIface.connector.TypeConverter()
	executionContext := lIface.connector.NewExecutionContext()
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.network.interfaces.link_local", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeLinkLocalNetworkInterface
	if methodResult.IsSuccess() {
