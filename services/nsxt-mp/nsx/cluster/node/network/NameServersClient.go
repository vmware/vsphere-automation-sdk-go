...
type NameServersClient interface {
	...
	Get(clusterNodeIdParam string) (nsxModel.NodeNameServersProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(clusterNodeIdParam string, nodeNameServersPropertiesParam nsxModel.NodeNameServersProperties) (nsxModel.NodeNameServersProperties, error)
}
...
func NewNameServersClient(connector vapiProtocolClient_.Connector) *nameServersClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.network.name_servers")
...
func (nIface *nameServersClient) Get(clusterNodeIdParam string) (nsxModel.NodeNameServersProperties, error) {
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.network.name_servers", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeNameServersProperties
...
func (nIface *nameServersClient) Update(clusterNodeIdParam string, nodeNameServersPropertiesParam nsxModel.NodeNameServersProperties) (nsxModel.NodeNameServersProperties, error) {
	typeConverter := nIface.connector.TypeConverter()
	executionContext := nIface.connector.NewExecutionContext()
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.network.name_servers", "update", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeNameServersProperties
	if methodResult.IsSuccess() {
