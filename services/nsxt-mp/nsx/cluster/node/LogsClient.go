...
type LogsClient interface {
	...
	Get(logNameParam string, clusterNodeIdParam string) (nsxModel.NodeLogProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(clusterNodeIdParam string) (nsxModel.NodeLogPropertiesListResult, error)
}
...
func NewLogsClient(connector vapiProtocolClient_.Connector) *logsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.logs")
...
func (lIface *logsClient) Get(logNameParam string, clusterNodeIdParam string) (nsxModel.NodeLogProperties, error) {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.logs", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeLogProperties
...
func (lIface *logsClient) List(clusterNodeIdParam string) (nsxModel.NodeLogPropertiesListResult, error) {
	typeConverter := lIface.connector.TypeConverter()
	executionContext := lIface.connector.NewExecutionContext()
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.logs", "list", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeLogPropertiesListResult
	if methodResult.IsSuccess() {
