...
type FormFactorsClient interface {
	...
	Get(clusterNodeIdParam string) (nsxModel.IntelligenceFormFactors, error)
}
...
func NewFormFactorsClient(connector vapiProtocolClient_.Connector) *formFactorsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.intelligence.form_factors")
...
func (fIface *formFactorsClient) Get(clusterNodeIdParam string) (nsxModel.IntelligenceFormFactors, error) {
	...
	methodResult := fIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.intelligence.form_factors", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.IntelligenceFormFactors
