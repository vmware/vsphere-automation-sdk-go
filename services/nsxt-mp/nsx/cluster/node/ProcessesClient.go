...
type ProcessesClient interface {
	...
	Get(processIdParam string, clusterNodeIdParam string) (nsxModel.NodeProcessProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(clusterNodeIdParam string) (nsxModel.NodeProcessPropertiesListResult, error)
}
...
func NewProcessesClient(connector vapiProtocolClient_.Connector) *processesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.processes")
...
func (pIface *processesClient) Get(processIdParam string, clusterNodeIdParam string) (nsxModel.NodeProcessProperties, error) {
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.processes", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeProcessProperties
...
func (pIface *processesClient) List(clusterNodeIdParam string) (nsxModel.NodeProcessPropertiesListResult, error) {
	typeConverter := pIface.connector.TypeConverter()
	executionContext := pIface.connector.NewExecutionContext()
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.processes", "list", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeProcessPropertiesListResult
	if methodResult.IsSuccess() {
