...
type VidmClient interface {
	...
	Get(clusterNodeIdParam string) (nsxModel.NodeAuthProviderVidmProperties, error)

	// Update AAA provider vIDM properties
	//  vIDM is deprecated by vIDB. Use API POST /api/v1/trust-management/oidc-uris/action/configure-vidb-oidc-endpoint.
	//
	// Deprecated: This API element is deprecated.
	//
	// @param clusterNodeIdParam (required)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(clusterNodeIdParam string, nodeAuthProviderVidmPropertiesParam nsxModel.NodeAuthProviderVidmProperties) (nsxModel.NodeAuthProviderVidmProperties, error)
}
...
func NewVidmClient(connector vapiProtocolClient_.Connector) *vidmClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.node.aaa.providers.vidm")
...
func (vIface *vidmClient) Get(clusterNodeIdParam string) (nsxModel.NodeAuthProviderVidmProperties, error) {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.aaa.providers.vidm", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeAuthProviderVidmProperties
...
func (vIface *vidmClient) Update(clusterNodeIdParam string, nodeAuthProviderVidmPropertiesParam nsxModel.NodeAuthProviderVidmProperties) (nsxModel.NodeAuthProviderVidmProperties, error) {
	typeConverter := vIface.connector.TypeConverter()
	executionContext := vIface.connector.NewExecutionContext()
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.node.aaa.providers.vidm", "update", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeAuthProviderVidmProperties
	if methodResult.IsSuccess() {
