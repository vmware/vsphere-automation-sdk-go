...
type ConcurrencyLimitingStatusClient interface {
	...
	// @return com.vmware.nsx.model.ConcurrencyLimitingStatus
	//
	...
	Get() (nsxModel.ConcurrencyLimitingStatus, error)
}
...
func NewConcurrencyLimitingStatusClient(connector vapiProtocolClient_.Connector) *concurrencyLimitingStatusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.cluster.api_service.concurrency_limiting_status")
...
func (cIface *concurrencyLimitingStatusClient) Get() (nsxModel.ConcurrencyLimitingStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.ConcurrencyLimitingStatus
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx.cluster.api_service.concurrency_limiting_status", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.ConcurrencyLimitingStatus
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.ConcurrencyLimitingStatus), nil
	} else {
