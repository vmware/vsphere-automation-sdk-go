...
type SessionsClient interface {
	...
	// @param ipsecVpnServiceIdParam All the IPSec VPN sessions configured with the specified VPN Service id will be included in the result. (optional)
	// @param logicalRouterIdParam All the VPN sessions belonging to the specified logical router id will be included in the result. (optional)
	// @param pageSizeParam Maximum number of results to return in this page (server may return fewer) (optional, default to 1000)
	// @param sessionTypeParam A Policy Based VPN requires to define protect rules that match local and peer subnets. IPSec security associations is negotiated for each pair of local and peer subnet. A Route Based VPN is more flexible, more powerful and recommended over policy based VPN. IP Tunnel port is created and all traffic routed via tunnel port is protected. Routes can be configured statically or can be learned through BGP. A route based VPN is must for establishing redundant VPN session to remote site. (optional)
