...
type LoglevelClient interface {
	...
	Get(transportNodeIdParam string) (nsxModel.NAPILogLevelValue, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(transportNodeIdParam string, nAPILogLevelValueParam nsxModel.NAPILogLevelValue) (nsxModel.NAPILogLevelValue, error)
}
...
func NewLoglevelClient(connector vapiProtocolClient_.Connector) *loglevelClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.services.node_mgmt.loglevel")
...
func (lIface *loglevelClient) Get(transportNodeIdParam string) (nsxModel.NAPILogLevelValue, error) {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.services.node_mgmt.loglevel", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NAPILogLevelValue
...
func (lIface *loglevelClient) Update(transportNodeIdParam string, nAPILogLevelValueParam nsxModel.NAPILogLevelValue) (nsxModel.NAPILogLevelValue, error) {
	typeConverter := lIface.connector.TypeConverter()
	executionContext := lIface.connector.NewExecutionContext()
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.services.node_mgmt.loglevel", "update", inputDataValue, executionContext)
	var emptyOutput nsxModel.NAPILogLevelValue
	if methodResult.IsSuccess() {
