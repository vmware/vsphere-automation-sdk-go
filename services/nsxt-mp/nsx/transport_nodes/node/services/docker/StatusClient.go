...
type StatusClient interface {
	...
	// Checks the status of dockerd process on the Edge. If dockerd process is running, returns \"running\", returns \"stopped\" otherwise.
	//
	...
	Get(transportNodeIdParam string) (nsxModel.NodeServiceStatusProperties, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.services.docker.status")
...
func (sIface *statusClient) Get(transportNodeIdParam string) (nsxModel.NodeServiceStatusProperties, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.services.docker.status", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeServiceStatusProperties
