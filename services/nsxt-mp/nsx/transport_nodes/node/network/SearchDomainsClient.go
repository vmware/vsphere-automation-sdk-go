...
type SearchDomainsClient interface {
	...
	Get(transportNodeIdParam string) (nsxModel.NodeSearchDomainsProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(transportNodeIdParam string, nodeSearchDomainsPropertiesParam nsxModel.NodeSearchDomainsProperties) (nsxModel.NodeSearchDomainsProperties, error)
}
...
func NewSearchDomainsClient(connector vapiProtocolClient_.Connector) *searchDomainsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.network.search_domains")
...
func (sIface *searchDomainsClient) Get(transportNodeIdParam string) (nsxModel.NodeSearchDomainsProperties, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.network.search_domains", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeSearchDomainsProperties
...
func (sIface *searchDomainsClient) Update(transportNodeIdParam string, nodeSearchDomainsPropertiesParam nsxModel.NodeSearchDomainsProperties) (nsxModel.NodeSearchDomainsProperties, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.network.search_domains", "update", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeSearchDomainsProperties
	if methodResult.IsSuccess() {
