...
type StatusClient interface {
	...
	Clearbootuperror(transportNodeIdParam string) (nsxModel.NsxNodeStatusProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Get(transportNodeIdParam string) (nsxModel.NsxNodeStatusProperties, error)
}
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.status")
...
func (sIface *statusClient) Clearbootuperror(transportNodeIdParam string) (nsxModel.NsxNodeStatusProperties, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.status", "clearbootuperror", inputDataValue, executionContext)
	var emptyOutput nsxModel.NsxNodeStatusProperties
...
func (sIface *statusClient) Get(transportNodeIdParam string) (nsxModel.NsxNodeStatusProperties, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.status", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NsxNodeStatusProperties
	if methodResult.IsSuccess() {
