...
type SshKeysClient interface {
	...
	Addsshkey(useridParam string, transportNodeIdParam string, sshKeyPropertiesParam nsxModel.SshKeyProperties) error
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(useridParam string, transportNodeIdParam string) (nsxModel.SshKeyPropertiesListResult, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Removesshkey(useridParam string, transportNodeIdParam string, sshKeyBasePropertiesParam nsxModel.SshKeyBaseProperties) error
}
...
func NewSshKeysClient(connector vapiProtocolClient_.Connector) *sshKeysClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.users.ssh_keys")
...
func (sIface *sshKeysClient) Addsshkey(useridParam string, transportNodeIdParam string, sshKeyPropertiesParam nsxModel.SshKeyProperties) error {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.users.ssh_keys", "addsshkey", inputDataValue, executionContext)
...
func (sIface *sshKeysClient) List(useridParam string, transportNodeIdParam string) (nsxModel.SshKeyPropertiesListResult, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.users.ssh_keys", "list", inputDataValue, executionContext)
	var emptyOutput nsxModel.SshKeyPropertiesListResult
	if methodResult.IsSuccess() {
...
func (sIface *sshKeysClient) Removesshkey(useridParam string, transportNodeIdParam string, sshKeyBasePropertiesParam nsxModel.SshKeyBaseProperties) error {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.users.ssh_keys", "removesshkey", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
