...
type ProcessesClient interface {
	...
	Get(processIdParam string, transportNodeIdParam string) (nsxModel.NodeProcessProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(transportNodeIdParam string) (nsxModel.NodeProcessPropertiesListResult, error)
}
...
func NewProcessesClient(connector vapiProtocolClient_.Connector) *processesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.processes")
...
func (pIface *processesClient) Get(processIdParam string, transportNodeIdParam string) (nsxModel.NodeProcessProperties, error) {
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.processes", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeProcessProperties
...
func (pIface *processesClient) List(transportNodeIdParam string) (nsxModel.NodeProcessPropertiesListResult, error) {
	typeConverter := pIface.connector.TypeConverter()
	executionContext := pIface.connector.NewExecutionContext()
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.processes", "list", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeProcessPropertiesListResult
	if methodResult.IsSuccess() {
