...
type ThumbprintClient interface {
	...
	Get(fileNameParam string, transportNodeIdParam string) (nsxModel.FileThumbprint, error)
}
...
func NewThumbprintClient(connector vapiProtocolClient_.Connector) *thumbprintClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.file_store.thumbprint")
...
func (tIface *thumbprintClient) Get(fileNameParam string, transportNodeIdParam string) (nsxModel.FileThumbprint, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.file_store.thumbprint", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.FileThumbprint
