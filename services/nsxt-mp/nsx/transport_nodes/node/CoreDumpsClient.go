...
func NewCoreDumpsClient(connector vapiProtocolClient_.Connector) *coreDumpsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.core_dumps")
...
func (cIface *coreDumpsClient) Copytoremotefile(fileNameParam string, transportNodeIdParam string, copyToRemoteFilePropertiesParam nsxModel.CopyToRemoteFileProperties) error {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.core_dumps", "copytoremotefile", inputDataValue, executionContext)
...
func (cIface *coreDumpsClient) Delete(fileNameParam string, transportNodeIdParam string) error {
	typeConverter := cIface.connector.TypeConverter()
	executionContext := cIface.connector.NewExecutionContext()
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.core_dumps", "delete", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (cIface *coreDumpsClient) List(transportNodeIdParam string) (nsxModel.FilePropertiesListResult, error) {
	typeConverter := cIface.connector.TypeConverter()
	executionContext := cIface.connector.NewExecutionContext()
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.core_dumps", "list", inputDataValue, executionContext)
	var emptyOutput nsxModel.FilePropertiesListResult
	if methodResult.IsSuccess() {
