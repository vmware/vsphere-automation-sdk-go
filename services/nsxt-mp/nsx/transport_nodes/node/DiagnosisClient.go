...
type DiagnosisClient interface {
	...
	// @return com.vmware.nsx.model.EdgeDiagnosis
	//
	...
	Get(transportNodeIdParam string) (nsxModel.EdgeDiagnosis, error)
}
...
func NewDiagnosisClient(connector vapiProtocolClient_.Connector) *diagnosisClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.diagnosis")
...
func (dIface *diagnosisClient) Get(transportNodeIdParam string) (nsxModel.EdgeDiagnosis, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.EdgeDiagnosis
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.diagnosis", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.EdgeDiagnosis
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.EdgeDiagnosis), nil
	} else {
