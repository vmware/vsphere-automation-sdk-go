...
type LogsClient interface {
	...
	Get(logNameParam string, transportNodeIdParam string) (nsxModel.NodeLogProperties, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(transportNodeIdParam string) (nsxModel.NodeLogPropertiesListResult, error)
}
...
func NewLogsClient(connector vapiProtocolClient_.Connector) *logsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.transport_nodes.node.logs")
...
func (lIface *logsClient) Get(logNameParam string, transportNodeIdParam string) (nsxModel.NodeLogProperties, error) {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.logs", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeLogProperties
...
func (lIface *logsClient) List(transportNodeIdParam string) (nsxModel.NodeLogPropertiesListResult, error) {
	typeConverter := lIface.connector.TypeConverter()
	executionContext := lIface.connector.NewExecutionContext()
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx.transport_nodes.node.logs", "list", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeLogPropertiesListResult
	if methodResult.IsSuccess() {
