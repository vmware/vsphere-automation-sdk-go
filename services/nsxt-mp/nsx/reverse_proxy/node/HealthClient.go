...
type HealthClient interface {
	...
	// @return com.vmware.nsx.model.NodeHealth
	//
...
func NewHealthClient(connector vapiProtocolClient_.Connector) *healthClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.reverse_proxy.node.health")
...
func (hIface *healthClient) Get() (nsxModel.NodeHealth, error) {
	...
	if inputError != nil {
		var emptyOutput nsxModel.NodeHealth
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := hIface.connector.GetApiProvider().Invoke("com.vmware.nsx.reverse_proxy.node.health", "get", inputDataValue, executionContext)
	var emptyOutput nsxModel.NodeHealth
	if methodResult.IsSuccess() {
		...
		return output.(nsxModel.NodeHealth), nil
	} else {
