...
func NewSensorRegistrationTokenClient(connector vapiProtocolClient_.Connector) *sensorRegistrationTokenClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx.security.sensor_registration_token")
...
func (sIface *sensorRegistrationTokenClient) Create() (nsxModel.RegistrationToken, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx.security.sensor_registration_token", "create", inputDataValue, executionContext)
	var emptyOutput nsxModel.RegistrationToken
