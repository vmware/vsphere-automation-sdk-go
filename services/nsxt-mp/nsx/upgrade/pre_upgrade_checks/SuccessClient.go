...
type SuccessClient interface {
	...
	//
	// @param componentTypeParam Component type on which upgrade check successes are to be filtered (optional)
	// @param cursorParam Opaque cursor to be used for getting next page of records (supplied by current result page) (optional)
	// @param filterTextParam Text to filter the results on. The filter text is matched with origin name and success message. String matching for the filter is case-insensitive. (optional)
	// @param includedFieldsParam Note - this parameter currently only works when used with the search APIs /policy/api/v1/search/query and /policy/api/v1/search/dsl. It is ignored for other list APIs. (optional)
	// @param originTypeParam Type of origin of pre/post-upgrade check success (optional)
