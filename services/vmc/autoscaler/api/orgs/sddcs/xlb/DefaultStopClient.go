...
/*
 * Client stubs for service: Stop
 * Functions that implement the generated StopClient interface
 */
...
func NewDefaultStopClient(connector client.Connector) *DefaultStopClient {
	interfaceName := "com.vmware.api.orgs.sddcs.xlb.stop"
	...
	sIface := DefaultStopClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	sIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	sIface.methodNameToDefMap["post"] = sIface.postMethodDefinition()
	return &sIface
}

func (sIface *DefaultStopClient) Post(orgParam string, sddcParam string) (model.Task, error) {
	typeConverter := sIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(sIface.interfaceIdentifier, "post")
	sv := bindings.NewStructValueBuilder(stopPostInputType(), typeConverter)
	...
	operationRestMetaData := stopPostRestMetadata()
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), stopPostOutputType())
...
func (sIface *DefaultStopClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := sIface.connector.GetApiProvider().Invoke(sIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (sIface *DefaultStopClient) postMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(sIface.interfaceName)
	typeConverter := sIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(stopPostInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(stopPostOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStopClient.post method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStopClient.post method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStopClient.post method's errors.Unauthenticated error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStopClient.post method's errors.InvalidRequest error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultStopClient.post method's errors.Unauthorized error - %s",
