...
/*
 * Client stubs for service: EdrsProvisioningSpec
 * Functions that implement the generated EdrsProvisioningSpecClient interface
 */
...
type DefaultEdrsProvisioningSpecClient struct {
...
func NewDefaultEdrsProvisioningSpecClient(connector client.Connector) *DefaultEdrsProvisioningSpecClient {
	interfaceName := "com.vmware.api.orgs.sddcs.clusters.edrs_provisioning_spec"
	...
	eIface := DefaultEdrsProvisioningSpecClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	eIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	eIface.methodNameToDefMap["get"] = eIface.getMethodDefinition()
	return &eIface
}

func (eIface *DefaultEdrsProvisioningSpecClient) Get(orgParam string, sddcParam string, clusterParam string) (model.EdrsProvisioningSpec, error) {
...
func (eIface *DefaultEdrsProvisioningSpecClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := eIface.connector.GetApiProvider().Invoke(eIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (eIface *DefaultEdrsProvisioningSpecClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(eIface.interfaceName)
	typeConverter := eIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(edrsProvisioningSpecGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(edrsProvisioningSpecGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultEdrsProvisioningSpecClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultEdrsProvisioningSpecClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultEdrsProvisioningSpecClient.get method's errors.Unauthenticated error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultEdrsProvisioningSpecClient.get method's errors.InvalidRequest error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultEdrsProvisioningSpecClient.get method's errors.Unauthorized error - %s",
