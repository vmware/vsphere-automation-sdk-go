...
/*
 * Client stubs for service: Reconfigure
 * Functions that implement the generated ReconfigureClient interface
 */
...
type DefaultReconfigureClient struct {
...
func NewDefaultReconfigureClient(connector client.Connector) *DefaultReconfigureClient {
	interfaceName := "com.vmware.vmc.orgs.sddcs.clusters.reconfigure"
	interfaceIdentifier := core.NewInterfaceIdentifier(interfaceName)
	methodIdentifiers := []core.MethodIdentifier{
		core.NewMethodIdentifier(interfaceIdentifier, "clusterReconfig"),
	}
	...
	rIface := DefaultReconfigureClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	rIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	rIface.methodNameToDefMap["cluster_reconfig"] = rIface.clusterReconfigMethodDefinition()
	return &rIface
}

func (rIface *DefaultReconfigureClient) ClusterReconfig(orgParam string, sddcParam string, clusterParam string, clusterReconfigureParamsParam model.ClusterReconfigureParams) (model.Task, error) {
	typeConverter := rIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(rIface.interfaceIdentifier, "cluster_reconfig")
...
func (rIface *DefaultReconfigureClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := rIface.connector.GetApiProvider().Invoke(rIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (rIface *DefaultReconfigureClient) clusterReconfigMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(rIface.interfaceName)
	typeConverter := rIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(reconfigureClusterReconfigInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(reconfigureClusterReconfigOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultReconfigureClient.clusterReconfig method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultReconfigureClient.clusterReconfig method's output - %s",
			bindings.VAPIerrorsToError(outputError).Error())
		return nil
	}
	methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "clusterReconfig")
	errorDefinitions := make([]data.ErrorDefinition, 0)
	rIface.errorBindingMap[errors.Unauthenticated{}.Error()] = errors.UnauthenticatedBindingType()
	errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.UnauthenticatedBindingType())
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultReconfigureClient.clusterReconfig method's errors.Unauthenticated error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultReconfigureClient.clusterReconfig method's errors.InvalidRequest error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultReconfigureClient.clusterReconfig method's errors.Unauthorized error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultReconfigureClient.clusterReconfig method's errors.NotFound error - %s",
