...
func NewL2vpnContextClient(connector client.Connector) *l2vpnContextClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.l2vpn_context")
...
func (lIface *l2vpnContextClient) Get(tier0IdParam string, localeServiceIdParam string) (model.L2VpnContext, error) {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.l2vpn_context", "get", inputDataValue, executionContext)
	var emptyOutput model.L2VpnContext
