...
func NewL3vpnContextClient(connector client.Connector) *l3vpnContextClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.l3vpn_context")
...
func (lIface *l3vpnContextClient) Get(tier0IdParam string, localeServiceIdParam string) (model.L3VpnContext, error) {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.l3vpn_context", "get", inputDataValue, executionContext)
	var emptyOutput model.L3VpnContext
