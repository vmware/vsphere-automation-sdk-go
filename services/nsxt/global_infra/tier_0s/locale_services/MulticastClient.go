...
func NewMulticastClient(connector client.Connector) *multicastClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.multicast")
...
func (mIface *multicastClient) Get(tier0IdParam string, localeServicesIdParam string) (model.PolicyMulticastConfig, error) {
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.multicast", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyMulticastConfig
...
func (mIface *multicastClient) Patch(tier0IdParam string, localeServicesIdParam string, policyMulticastConfigParam model.PolicyMulticastConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	mIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.multicast", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), mIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (mIface *multicastClient) Update(tier0IdParam string, localeServicesIdParam string, policyMulticastConfigParam model.PolicyMulticastConfig) (model.PolicyMulticastConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	mIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.multicast", "update", inputDataValue, executionContext)
	var emptyOutput model.PolicyMulticastConfig
	if methodResult.IsSuccess() {
