...
func NewStatisticsClient(connector client.Connector) *statisticsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.l3vpns.statistics")
...
func (sIface *statisticsClient) Get(tier0IdParam string, localeServiceIdParam string, l3vpnIdParam string, enforcementPointPathParam *string) (model.AggregateL3VpnStatistics, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.locale_services.l3vpns.statistics", "get", inputDataValue, executionContext)
	var emptyOutput model.AggregateL3VpnStatistics
