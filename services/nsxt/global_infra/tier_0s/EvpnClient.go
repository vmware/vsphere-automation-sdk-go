...
func NewEvpnClient(connector client.Connector) *evpnClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_0s.evpn")
...
func (eIface *evpnClient) Get(tier0IdParam string) (model.EvpnConfig, error) {
	...
	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.evpn", "get", inputDataValue, executionContext)
	var emptyOutput model.EvpnConfig
...
func (eIface *evpnClient) Patch(tier0IdParam string, evpnConfigParam model.EvpnConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	eIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.evpn", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), eIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (eIface *evpnClient) Update(tier0IdParam string, evpnConfigParam model.EvpnConfig) (model.EvpnConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	eIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.evpn", "update", inputDataValue, executionContext)
	var emptyOutput model.EvpnConfig
	if methodResult.IsSuccess() {
