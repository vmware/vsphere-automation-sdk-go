...
func (nIface *natRulesClient) Delete(tier0IdParam string, natIdParam string, natRuleIdParam string) error {
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.nat.nat_rules", "delete", inputDataValue, executionContext)
...
func (nIface *natRulesClient) Patch(tier0IdParam string, natIdParam string, natRuleIdParam string, policyNatRuleParam model.PolicyNatRule) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	nIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.nat.nat_rules", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), nIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (nIface *natRulesClient) Update(tier0IdParam string, natIdParam string, natRuleIdParam string, policyNatRuleParam model.PolicyNatRule) (model.PolicyNatRule, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	nIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.nat.nat_rules", "update", inputDataValue, executionContext)
	var emptyOutput model.PolicyNatRule
	if methodResult.IsSuccess() {
