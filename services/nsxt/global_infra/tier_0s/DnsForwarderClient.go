...
func (dIface *dnsForwarderClient) Delete(tier0IdParam string) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.dns_forwarder", "delete", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), dIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (dIface *dnsForwarderClient) Patch(tier0IdParam string, policyDnsForwarderParam model.PolicyDnsForwarder) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.dns_forwarder", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), dIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (dIface *dnsForwarderClient) Update(tier0IdParam string, policyDnsForwarderParam model.PolicyDnsForwarder) (model.PolicyDnsForwarder, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_0s.dns_forwarder", "update", inputDataValue, executionContext)
	var emptyOutput model.PolicyDnsForwarder
	if methodResult.IsSuccess() {
