...
func (cIface *crlsClient) Delete(crlIdParam string) error {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.crls", "delete", inputDataValue, executionContext)
...
func (cIface *crlsClient) Importcrl(crlIdParam string, tlsCrlParam model.TlsCrl) (model.TlsCrlListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.crls", "importcrl", inputDataValue, executionContext)
	var emptyOutput model.TlsCrlListResult
	if methodResult.IsSuccess() {
...
func (cIface *crlsClient) Patch(crlIdParam string, tlsCrlParam model.TlsCrl) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.crls", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (cIface *crlsClient) Update(crlIdParam string, tlsCrlParam model.TlsCrl) (model.TlsCrl, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.crls", "update", inputDataValue, executionContext)
	var emptyOutput model.TlsCrl
	if methodResult.IsSuccess() {
