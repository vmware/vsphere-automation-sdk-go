...
type HeartbeatConfigClient interface {
	...
	Get() (model.AntreaHeartbeatConfig, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Patch(antreaHeartbeatConfigParam model.AntreaHeartbeatConfig) error
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Update(antreaHeartbeatConfigParam model.AntreaHeartbeatConfig) (model.AntreaHeartbeatConfig, error)
}
...
func NewHeartbeatConfigClient(connector client.Connector) *heartbeatConfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.antrea.heartbeat_config")
...
func (hIface *heartbeatConfigClient) Get() (model.AntreaHeartbeatConfig, error) {
	...
	methodResult := hIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.antrea.heartbeat_config", "get", inputDataValue, executionContext)
	var emptyOutput model.AntreaHeartbeatConfig
...
func (hIface *heartbeatConfigClient) Patch(antreaHeartbeatConfigParam model.AntreaHeartbeatConfig) error {
	typeConverter := hIface.connector.TypeConverter()
	executionContext := hIface.connector.NewExecutionContext()
	...
	connectionMetadata["isStreamingResponse"] = false
	hIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := hIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.antrea.heartbeat_config", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), hIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (hIface *heartbeatConfigClient) Update(antreaHeartbeatConfigParam model.AntreaHeartbeatConfig) (model.AntreaHeartbeatConfig, error) {
	typeConverter := hIface.connector.TypeConverter()
	executionContext := hIface.connector.NewExecutionContext()
	...
	connectionMetadata["isStreamingResponse"] = false
	hIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := hIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.antrea.heartbeat_config", "update", inputDataValue, executionContext)
	var emptyOutput model.AntreaHeartbeatConfig
	if methodResult.IsSuccess() {
