...
func NewIpPoolsClient(connector client.Connector) *ipPoolsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.ip_pools")
...
func (iIface *ipPoolsClient) Delete(ipPoolIdParam string) error {
	...
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools", "delete", inputDataValue, executionContext)
...
func (iIface *ipPoolsClient) Get(ipPoolIdParam string) (model.IpAddressPool, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools", "get", inputDataValue, executionContext)
	var emptyOutput model.IpAddressPool
	if methodResult.IsSuccess() {
...
func (iIface *ipPoolsClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.IpAddressPoolListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools", "list", inputDataValue, executionContext)
	var emptyOutput model.IpAddressPoolListResult
	if methodResult.IsSuccess() {
...
func (iIface *ipPoolsClient) Patch(ipPoolIdParam string, ipAddressPoolParam model.IpAddressPool) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), iIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (iIface *ipPoolsClient) Update(ipPoolIdParam string, ipAddressPoolParam model.IpAddressPool) (model.IpAddressPool, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools", "update", inputDataValue, executionContext)
	var emptyOutput model.IpAddressPool
	if methodResult.IsSuccess() {
