...
/*
 * Client stubs for service: Aggregated_with_pagination
 * Functions that implement the generated Aggregated_with_paginationClient interface
 */
...
type DefaultAggregated_with_paginationClient struct {
...
func NewDefaultAggregated_with_paginationClient(connector client.Connector) *DefaultAggregated_with_paginationClient {
	interfaceName := "com.vmware.nsx_policy.global_infra.drafts.aggregated_with_pagination"
	...
	aIface := DefaultAggregated_with_paginationClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	aIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	aIface.methodNameToDefMap["get"] = aIface.getMethodDefinition()
	return &aIface
}

func (aIface *DefaultAggregated_with_paginationClient) Get(draftIdParam string, requestIdParam *string, rootPathParam *string) (model.PolicyDraftPaginatedAggregatedConfigurationResult, error) {
...
func (aIface *DefaultAggregated_with_paginationClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := aIface.connector.GetApiProvider().Invoke(aIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (aIface *DefaultAggregated_with_paginationClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(aIface.interfaceName)
	typeConverter := aIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(aggregatedWithPaginationGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(aggregatedWithPaginationGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultAggregated_with_paginationClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultAggregated_with_paginationClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultAggregated_with_paginationClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultAggregated_with_paginationClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultAggregated_with_paginationClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultAggregated_with_paginationClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultAggregated_with_paginationClient.get method's errors.NotFound error - %s",
