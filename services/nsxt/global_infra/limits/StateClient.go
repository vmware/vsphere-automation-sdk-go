...
type StateClient interface {
	...
	Get(limitIdParam string) (nsx_policyModel.LimitState, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	List(cursorParam *string, includeConflictsParam *bool, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.LimitStateListResult, error)
}
...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.limits.state")
...
func (sIface *stateClient) Get(limitIdParam string) (nsx_policyModel.LimitState, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.LimitState
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.limits.state", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.LimitState
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.LimitState), nil
	} else {
...
func (sIface *stateClient) List(cursorParam *string, includeConflictsParam *bool, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.LimitStateListResult, error) {
	typeConverter := sIface.connector.TypeConverter()
	executionContext := sIface.connector.NewExecutionContext()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		var emptyOutput nsx_policyModel.LimitStateListResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.limits.state", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.LimitStateListResult
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.LimitStateListResult), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
		if errorInError != nil {
			return emptyOutput, vapiBindings_.VAPIerrorsToError(errorInError)
