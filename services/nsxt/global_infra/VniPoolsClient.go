...
func NewVniPoolsClient(connector client.Connector) *vniPoolsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.vni_pools")
...
func (vIface *vniPoolsClient) Delete(vniPoolIdParam string) error {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.vni_pools", "delete", inputDataValue, executionContext)
...
func (vIface *vniPoolsClient) Get(vniPoolIdParam string) (model.VniPoolConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	vIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.vni_pools", "get", inputDataValue, executionContext)
	var emptyOutput model.VniPoolConfig
	if methodResult.IsSuccess() {
...
func (vIface *vniPoolsClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.VniPoolConfigListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	vIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.vni_pools", "list", inputDataValue, executionContext)
	var emptyOutput model.VniPoolConfigListResult
	if methodResult.IsSuccess() {
...
func (vIface *vniPoolsClient) Patch(vniPoolIdParam string, vniPoolConfigParam model.VniPoolConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	vIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.vni_pools", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), vIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (vIface *vniPoolsClient) Update(vniPoolIdParam string, vniPoolConfigParam model.VniPoolConfig) (model.VniPoolConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	vIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.vni_pools", "update", inputDataValue, executionContext)
	var emptyOutput model.VniPoolConfig
	if methodResult.IsSuccess() {
