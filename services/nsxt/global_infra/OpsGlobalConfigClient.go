...
func NewOpsGlobalConfigClient(connector client.Connector) *opsGlobalConfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.ops_global_config")
...
func (oIface *opsGlobalConfigClient) Get() (model.OpsGlobalConfig, error) {
	...
	methodResult := oIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ops_global_config", "get", inputDataValue, executionContext)
	var emptyOutput model.OpsGlobalConfig
...
func (oIface *opsGlobalConfigClient) Patch(opsGlobalConfigParam model.OpsGlobalConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	oIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := oIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ops_global_config", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), oIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (oIface *opsGlobalConfigClient) Update(opsGlobalConfigParam model.OpsGlobalConfig) (model.OpsGlobalConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	oIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := oIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ops_global_config", "update", inputDataValue, executionContext)
	var emptyOutput model.OpsGlobalConfig
	if methodResult.IsSuccess() {
