...
func NewVirtualMachinesClient(connector client.Connector) *virtualMachinesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.realized_state.enforcement_points.virtual_machines")
...
func (vIface *virtualMachinesClient) List(enforcementPointNameParam string, cursorParam *string, dslParam *string, includedFieldsParam *string, pageSizeParam *int64, queryParam *string, sortAscendingParam *bool, sortByParam *string) (model.SearchResponse, error) {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.realized_state.enforcement_points.virtual_machines", "list", inputDataValue, executionContext)
	var emptyOutput model.SearchResponse
...
func (vIface *virtualMachinesClient) Updatetags(enforcementPointNameParam string, virtualMachineTagsUpdateParam model.VirtualMachineTagsUpdate) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	vIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.realized_state.enforcement_points.virtual_machines", "updatetags", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), vIface.GetErrorBindingType(methodResult.Error().Name()))
