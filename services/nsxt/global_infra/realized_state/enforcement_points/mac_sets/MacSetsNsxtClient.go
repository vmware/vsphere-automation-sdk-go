...
func NewMacSetsNsxtClient(connector client.Connector) *macSetsNsxtClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.realized_state.enforcement_points.mac_sets.mac_sets_nsxt")
...
func (mIface *macSetsNsxtClient) Get(enforcementPointNameParam string, macSetNameParam string) (model.GenericPolicyRealizedResource, error) {
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.realized_state.enforcement_points.mac_sets.mac_sets_nsxt", "get", inputDataValue, executionContext)
	var emptyOutput model.GenericPolicyRealizedResource
...
func (mIface *macSetsNsxtClient) List(enforcementPointNameParam string, cursorParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.GenericPolicyRealizedResourceListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	mIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.realized_state.enforcement_points.mac_sets.mac_sets_nsxt", "list", inputDataValue, executionContext)
	var emptyOutput model.GenericPolicyRealizedResourceListResult
	if methodResult.IsSuccess() {
