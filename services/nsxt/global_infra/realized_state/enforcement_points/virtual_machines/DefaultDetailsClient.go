...
func NewDefaultDetailsClient(connector client.Connector) *DefaultDetailsClient {
	interfaceName := "com.vmware.nsx_policy.global_infra.realized_state.enforcement_points.virtual_machines.details"
...
func (dIface *DefaultDetailsClient) Get(enforcementPointNameParam string, virtualMachineIdParam string) (model.VirtualMachineDetails, error) {
...
func (dIface *DefaultDetailsClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(dIface.interfaceName)
	typeConverter := dIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(detailsGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(detailsGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDetailsClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDetailsClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDetailsClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDetailsClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDetailsClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDetailsClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultDetailsClient.get method's errors.NotFound error - %s",
