...
func NewGlobalConfigClient(connector client.Connector) *globalConfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.global_config")
...
func (gIface *globalConfigClient) Get() (model.GlobalConfig, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.global_config", "get", inputDataValue, executionContext)
	var emptyOutput model.GlobalConfig
...
func (gIface *globalConfigClient) Patch(globalConfigParam model.GlobalConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	gIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.global_config", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), gIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (gIface *globalConfigClient) Update(globalConfigParam model.GlobalConfig) (model.GlobalConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	gIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.global_config", "update", inputDataValue, executionContext)
	var emptyOutput model.GlobalConfig
	if methodResult.IsSuccess() {
