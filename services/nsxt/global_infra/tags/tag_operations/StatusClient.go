...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tags.tag_operations.status")
...
func (sIface *statusClient) Get(operationIdParam string) (model.TagBulkOperationStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tags.tag_operations.status", "get", inputDataValue, executionContext)
	var emptyOutput model.TagBulkOperationStatus
