...
func (tIface *traceflowsClient) Create(traceflowIdParam string, actionParam *string) (model.TraceflowConfig, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.traceflows", "create", inputDataValue, executionContext)
	var emptyOutput model.TraceflowConfig
...
func (tIface *traceflowsClient) Patch(traceflowIdParam string, traceflowConfigParam model.TraceflowConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.traceflows", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), tIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (tIface *traceflowsClient) Update(traceflowIdParam string, traceflowConfigParam model.TraceflowConfig) (model.TraceflowConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.traceflows", "update", inputDataValue, executionContext)
	var emptyOutput model.TraceflowConfig
	if methodResult.IsSuccess() {
