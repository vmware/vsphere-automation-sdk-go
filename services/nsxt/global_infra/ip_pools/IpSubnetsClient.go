...
func NewIpSubnetsClient(connector client.Connector) *ipSubnetsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.ip_pools.ip_subnets")
...
func (iIface *ipSubnetsClient) Delete(ipPoolIdParam string, ipSubnetIdParam string) error {
	...
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools.ip_subnets", "delete", inputDataValue, executionContext)
...
func (iIface *ipSubnetsClient) Get(ipPoolIdParam string, ipSubnetIdParam string) (*data.StructValue, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools.ip_subnets", "get", inputDataValue, executionContext)
	var emptyOutput *data.StructValue
	if methodResult.IsSuccess() {
...
func (iIface *ipSubnetsClient) List(ipPoolIdParam string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.IpAddressPoolSubnetListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools.ip_subnets", "list", inputDataValue, executionContext)
	var emptyOutput model.IpAddressPoolSubnetListResult
	if methodResult.IsSuccess() {
...
func (iIface *ipSubnetsClient) Patch(ipPoolIdParam string, ipSubnetIdParam string, ipAddressPoolSubnetParam *data.StructValue) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools.ip_subnets", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), iIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (iIface *ipSubnetsClient) Update(ipPoolIdParam string, ipSubnetIdParam string, ipAddressPoolSubnetParam *data.StructValue) (*data.StructValue, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.ip_pools.ip_subnets", "update", inputDataValue, executionContext)
	var emptyOutput *data.StructValue
	if methodResult.IsSuccess() {
