...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.firewall_identity_store_event_log_servers.status")
...
func (sIface *statusClient) Create(directoryEventLogServerParam nsx_policyModel.DirectoryEventLogServer, enforcementPointPathParam *string) (nsx_policyModel.DirectoryEventLogServerStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.firewall_identity_store_event_log_servers.status", "create", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.DirectoryEventLogServerStatus
