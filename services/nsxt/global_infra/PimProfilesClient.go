...
func NewPimProfilesClient(connector client.Connector) *pimProfilesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.pim_profiles")
...
func (pIface *pimProfilesClient) Delete(pimProfileIdParam string) error {
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.pim_profiles", "delete", inputDataValue, executionContext)
...
func (pIface *pimProfilesClient) Get(pimProfileIdParam string) (model.PolicyPimProfile, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.pim_profiles", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyPimProfile
	if methodResult.IsSuccess() {
...
func (pIface *pimProfilesClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.PolicyPimProfileListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.pim_profiles", "list", inputDataValue, executionContext)
	var emptyOutput model.PolicyPimProfileListResult
	if methodResult.IsSuccess() {
...
func (pIface *pimProfilesClient) Patch(pimProfileIdParam string, policyPimProfileParam model.PolicyPimProfile) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.pim_profiles", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), pIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (pIface *pimProfilesClient) Update(pimProfileIdParam string, policyPimProfileParam model.PolicyPimProfile) (model.PolicyPimProfile, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.pim_profiles", "update", inputDataValue, executionContext)
	var emptyOutput model.PolicyPimProfile
	if methodResult.IsSuccess() {
