...
func NewHostTransportNodesAggstatusClient(connector client.Connector) *hostTransportNodesAggstatusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.host_transport_nodes_aggstatus")
...
func (hIface *hostTransportNodesAggstatusClient) Get(siteIdParam string, enforcementPointIdParam string, nodeTypeParam *string) (model.HeatMapTransportZoneStatus, error) {
	...
	methodResult := hIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.host_transport_nodes_aggstatus", "get", inputDataValue, executionContext)
	var emptyOutput model.HeatMapTransportZoneStatus
