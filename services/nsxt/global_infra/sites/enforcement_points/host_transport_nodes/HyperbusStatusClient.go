...
func NewHyperbusStatusClient(connector client.Connector) *hyperbusStatusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.host_transport_nodes.hyperbus_status")
...
func (hIface *hyperbusStatusClient) Get(siteIdParam string, enforcementPointIdParam string, nodeIdParam string) (model.TnHyperbusStatus, error) {
	...
	methodResult := hIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.host_transport_nodes.hyperbus_status", "get", inputDataValue, executionContext)
	var emptyOutput model.TnHyperbusStatus
