...
func NewNcpstatusClient(connector client.Connector) *ncpstatusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.container_cluster.ncpstatus")
...
func (nIface *ncpstatusClient) Delete(siteIdParam string, enforcementPointIdParam string, clusterIdParam string) error {
	...
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.container_cluster.ncpstatus", "delete", inputDataValue, executionContext)
...
func (nIface *ncpstatusClient) Get(siteIdParam string, enforcementPointIdParam string, clusterIdParam string) (model.ContainerClusterSummary, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	nIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := nIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.container_cluster.ncpstatus", "get", inputDataValue, executionContext)
	var emptyOutput model.ContainerClusterSummary
	if methodResult.IsSuccess() {
