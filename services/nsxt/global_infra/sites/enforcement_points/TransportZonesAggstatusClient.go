...
func NewTransportZonesAggstatusClient(connector client.Connector) *transportZonesAggstatusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.transport_zones_aggstatus")
...
func (tIface *transportZonesAggstatusClient) Get(siteIdParam string, enforcementPointIdParam string) (model.HeatMapTransportNodesAggregateStatus, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.transport_zones_aggstatus", "get", inputDataValue, executionContext)
	var emptyOutput model.HeatMapTransportNodesAggregateStatus
