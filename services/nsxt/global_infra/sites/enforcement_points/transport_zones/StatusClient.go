...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.transport_zones.status")
...
func (sIface *statusClient) Get(siteIdParam string, enforcementPointIdParam string, zoneIdParam string, sourceParam *string) (model.HeatMapTransportZoneStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.transport_zones.status", "get", inputDataValue, executionContext)
	var emptyOutput model.HeatMapTransportZoneStatus
