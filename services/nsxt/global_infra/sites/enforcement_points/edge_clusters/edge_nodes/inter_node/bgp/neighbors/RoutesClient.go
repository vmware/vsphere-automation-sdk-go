...
type RoutesClient interface {
	...
	// Returns inter-node routes learned by BGP neighbor from the given edge transport node. It always returns realtime response.
	//
...
func NewRoutesClient(connector vapiProtocolClient_.Connector) *routesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.edge_clusters.edge_nodes.inter_node.bgp.neighbors.routes")
...
func (rIface *routesClient) Get(siteIdParam string, enforcementpointIdParam string, edgeClusterIdParam string, policyEdgeNodeIdParam string, neighborIdParam string) (nsx_policyModel.PolicyEdgeNodeBgpNeighborRouteDetailList, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.edge_clusters.edge_nodes.inter_node.bgp.neighbors.routes", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyEdgeNodeBgpNeighborRouteDetailList
