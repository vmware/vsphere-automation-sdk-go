...
type AdvertisedRoutesClient interface {
	...
	// Returns inter-node routes advertised by BGP neighbor from the given edge transport node.
	//
...
func NewAdvertisedRoutesClient(connector vapiProtocolClient_.Connector) *advertisedRoutesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.edge_clusters.edge_nodes.inter_node.bgp.neighbors.advertised_routes")
...
func (aIface *advertisedRoutesClient) Get(siteIdParam string, enforcementpointIdParam string, edgeClusterIdParam string, policyEdgeNodeIdParam string, neighborIdParam string) (nsx_policyModel.PolicyEdgeNodeBgpNeighborAdvertisedRoutes, error) {
	...
	methodResult := aIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.edge_clusters.edge_nodes.inter_node.bgp.neighbors.advertised_routes", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyEdgeNodeBgpNeighborAdvertisedRoutes
