...
func NewSummaryClient(connector vapiProtocolClient_.Connector) *summaryClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.edge_clusters.remote_tep_connectivity.bgp.summary")
...
func (sIface *summaryClient) List(siteIdParam string, enforcementpointIdParam string, edgeClusterIdParam string) (nsx_policyModel.PolicyEdgeClusterRemoteTunnelBgpSummary, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.edge_clusters.remote_tep_connectivity.bgp.summary", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyEdgeClusterRemoteTunnelBgpSummary
