...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.transport_node_collections.state")
...
func (sIface *stateClient) Get(siteIdParam string, enforcementpointIdParam string, transportNodeCollectionIdParam string) (model.TransportNodeCollectionState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.transport_node_collections.state", "get", inputDataValue, executionContext)
	var emptyOutput model.TransportNodeCollectionState
