...
func NewDiscoveredNodesClient(connector client.Connector) *discoveredNodesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.enforcement_points.fabric.discovered_nodes")
...
func (dIface *discoveredNodesClient) Createtransportnode(siteIdParam string, enforcementpointIdParam string, nodeExtIdParam string, hostTransportNodeParam model.HostTransportNode) (model.HostTransportNode, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.fabric.discovered_nodes", "createtransportnode", inputDataValue, executionContext)
	var emptyOutput model.HostTransportNode
...
func (dIface *discoveredNodesClient) Reapplyclusterconfig(siteIdParam string, enforcementpointIdParam string, nodeExtIdParam string) (model.HostTransportNode, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.enforcement_points.fabric.discovered_nodes", "reapplyclusterconfig", inputDataValue, executionContext)
	var emptyOutput model.HostTransportNode
	if methodResult.IsSuccess() {
