...
func NewKubeconfigClient(connector client.Connector) *kubeconfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.kubeconfig")
...
func (kIface *kubeconfigClient) Get(siteIdParam string) (model.KubeconfigInfo, error) {
	...
	methodResult := kIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.kubeconfig", "get", inputDataValue, executionContext)
	var emptyOutput model.KubeconfigInfo
