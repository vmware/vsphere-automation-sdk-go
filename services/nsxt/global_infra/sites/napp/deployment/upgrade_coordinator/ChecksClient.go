...
type ChecksClient interface {
	...
	// Deploy or Undeploy NSX Application Platform upgrade-coordinator
	//
...
func NewChecksClient(connector client.Connector) *checksClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.upgrade_coordinator.checks")
...
func (cIface *checksClient) Update(siteIdParam string, deploymentChecksActionParam model.DeploymentChecksAction) error {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.upgrade_coordinator.checks", "update", inputDataValue, executionContext)
