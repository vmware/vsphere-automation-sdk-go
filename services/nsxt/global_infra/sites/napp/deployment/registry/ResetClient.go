...
func NewResetClient(connector client.Connector) *resetClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.registry.reset")
...
func (rIface *resetClient) Update(siteIdParam string) (model.DeploymentRegistry, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.registry.reset", "update", inputDataValue, executionContext)
	var emptyOutput model.DeploymentRegistry
