...
func NewKubernetesToolsClient(connector client.Connector) *kubernetesToolsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.kubernetes_tools")
...
func (kIface *kubernetesToolsClient) Get(siteIdParam string) (model.KubernetesToolsInfo, error) {
	...
	methodResult := kIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.kubernetes_tools", "get", inputDataValue, executionContext)
	var emptyOutput model.KubernetesToolsInfo
