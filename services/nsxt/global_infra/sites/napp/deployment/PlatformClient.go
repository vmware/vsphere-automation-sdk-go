...
func NewPlatformClient(connector client.Connector) *platformClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform")
...
func (pIface *platformClient) Get(siteIdParam string) (model.PlatformDeploymentConfig, error) {
	...
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform", "get", inputDataValue, executionContext)
	var emptyOutput model.PlatformDeploymentConfig
...
func (pIface *platformClient) Patch(siteIdParam string, platformDeploymentConfigParam model.PlatformDeploymentConfig) (model.PlatformDeploymentConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	pIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := pIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform", "patch", inputDataValue, executionContext)
	var emptyOutput model.PlatformDeploymentConfig
	if methodResult.IsSuccess() {
