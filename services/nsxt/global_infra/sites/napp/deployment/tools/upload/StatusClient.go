...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.tools.upload.status")
...
func (sIface *statusClient) Get(siteIdParam string, bundleIdParam string) (model.UploadBundleStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.tools.upload.status", "get", inputDataValue, executionContext)
	var emptyOutput model.UploadBundleStatus
