...
func NewUpload_from_remote_serverClient(connector client.Connector) *upload_from_remote_serverClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.tools.upload_from_remote_server")
...
func (uIface *upload_from_remote_serverClient) Create(siteIdParam string, uploadBundleRemoteServerParam model.UploadBundleRemoteServer) (model.UploadBundleId, error) {
	...
	methodResult := uIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.tools.upload_from_remote_server", "create", inputDataValue, executionContext)
	var emptyOutput model.UploadBundleId
