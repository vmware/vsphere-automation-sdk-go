...
func NewChecksClient(connector client.Connector) *checksClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform.checks")
...
func (cIface *checksClient) Update(siteIdParam string, platformDeploymentChecksActionParam model.PlatformDeploymentChecksAction) error {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform.checks", "update", inputDataValue, executionContext)
