...
func NewFormFactorsClient(connector client.Connector) *formFactorsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform.form_factors")
...
func (fIface *formFactorsClient) Get(siteIdParam string) (model.PlatformFormFactors, error) {
	...
	methodResult := fIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform.form_factors", "get", inputDataValue, executionContext)
	var emptyOutput model.PlatformFormFactors
