...
func NewVersionsClient(connector client.Connector) *versionsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform.versions")
...
func (vIface *versionsClient) Get(siteIdParam string) (model.DeploymentVersions, error) {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.platform.versions", "get", inputDataValue, executionContext)
	var emptyOutput model.DeploymentVersions
