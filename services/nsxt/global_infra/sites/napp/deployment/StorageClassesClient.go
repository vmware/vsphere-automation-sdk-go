...
func NewStorageClassesClient(connector client.Connector) *storageClassesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.storage_classes")
...
func (sIface *storageClassesClient) List(siteIdParam string) (model.StorageClasses, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.storage_classes", "list", inputDataValue, executionContext)
	var emptyOutput model.StorageClasses
