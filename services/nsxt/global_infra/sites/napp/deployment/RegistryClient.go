...
func NewRegistryClient(connector client.Connector) *registryClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.sites.napp.deployment.registry")
...
func (rIface *registryClient) Get(siteIdParam string) (model.DeploymentRegistry, error) {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.registry", "get", inputDataValue, executionContext)
	var emptyOutput model.DeploymentRegistry
...
func (rIface *registryClient) Update(siteIdParam string, deploymentRegistryParam model.DeploymentRegistry) (model.DeploymentRegistry, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.sites.napp.deployment.registry", "update", inputDataValue, executionContext)
	var emptyOutput model.DeploymentRegistry
	if methodResult.IsSuccess() {
