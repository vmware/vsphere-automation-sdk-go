...
func (gIface *globalManagerConfigClient) Patch(globalManagerConfigParam model.GlobalManagerConfig) error {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.global_manager_config", "patch", inputDataValue, executionContext)
...
func (gIface *globalManagerConfigClient) Update(globalManagerConfigParam model.GlobalManagerConfig) (model.GlobalManagerConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	gIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.global_manager_config", "update", inputDataValue, executionContext)
	var emptyOutput model.GlobalManagerConfig
	if methodResult.IsSuccess() {
