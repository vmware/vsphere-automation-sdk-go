...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.service_insertion.security.status")
...
func (sIface *statusClient) Get() (model.PolicySIStatusConfiguration, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.service_insertion.security.status", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicySIStatusConfiguration
...
func (sIface *statusClient) Patch(policySIStatusConfigurationParam model.PolicySIStatusConfiguration) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.service_insertion.security.status", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (sIface *statusClient) Update(policySIStatusConfigurationParam model.PolicySIStatusConfiguration) (model.PolicySIStatusConfiguration, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.service_insertion.security.status", "update", inputDataValue, executionContext)
	var emptyOutput model.PolicySIStatusConfiguration
	if methodResult.IsSuccess() {
