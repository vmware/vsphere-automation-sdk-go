...
func NewIntrusionServicesClient(connector client.Connector) *intrusionServicesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services")
...
func (iIface *intrusionServicesClient) Get() (model.IdsSettings, error) {
	...
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services", "get", inputDataValue, executionContext)
	var emptyOutput model.IdsSettings
...
func (iIface *intrusionServicesClient) Patch(idsSettingsParam model.IdsSettings) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), iIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (iIface *intrusionServicesClient) Update(idsSettingsParam model.IdsSettings) (model.IdsSettings, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services", "update", inputDataValue, executionContext)
	var emptyOutput model.IdsSettings
	if methodResult.IsSuccess() {
