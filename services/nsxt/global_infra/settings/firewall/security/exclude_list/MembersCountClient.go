...
func NewMembersCountClient(connector vapiProtocolClient_.Connector) *membersCountClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.exclude_list.members_count")
...
func (mIface *membersCountClient) List() (nsx_policyModel.FirewallExcludeListMembersCountListResult, error) {
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.exclude_list.members_count", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.FirewallExcludeListMembersCountListResult
