...
func NewIdsEventsByCategoryClient(connector client.Connector) *idsEventsByCategoryClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_events_by_category")
...
func (iIface *idsEventsByCategoryClient) List(idsCategoryNameParam string, timeperiodParam string, detectedOnGatewayParam *bool, gatewayParam *string) (model.PolicyIdsEventsByCategoryResult, error) {
	...
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_events_by_category", "list", inputDataValue, executionContext)
	var emptyOutput model.PolicyIdsEventsByCategoryResult
