...
func NewGlobalEventConfigClient(connector client.Connector) *globalEventConfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.global_event_config")
...
func (gIface *globalEventConfigClient) Get() (model.IdsGlobalEventConfig, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.global_event_config", "get", inputDataValue, executionContext)
	var emptyOutput model.IdsGlobalEventConfig
...
func (gIface *globalEventConfigClient) Patch(idsGlobalEventConfigParam model.IdsGlobalEventConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	gIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.global_event_config", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), gIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (gIface *globalEventConfigClient) Update(idsGlobalEventConfigParam model.IdsGlobalEventConfig) (model.IdsGlobalEventConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	gIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.global_event_config", "update", inputDataValue, executionContext)
	var emptyOutput model.IdsGlobalEventConfig
	if methodResult.IsSuccess() {
