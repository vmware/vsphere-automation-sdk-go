...
func NewClusterConfigsClient(connector client.Connector) *clusterConfigsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.cluster_configs")
...
func (cIface *clusterConfigsClient) Get(clusterIdParam string) (model.IdsClusterConfig, error) {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.cluster_configs", "get", inputDataValue, executionContext)
	var emptyOutput model.IdsClusterConfig
...
func (cIface *clusterConfigsClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.IdsClusterConfigListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.cluster_configs", "list", inputDataValue, executionContext)
	var emptyOutput model.IdsClusterConfigListResult
	if methodResult.IsSuccess() {
...
func (cIface *clusterConfigsClient) Patch(clusterIdParam string, idsClusterConfigParam model.IdsClusterConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.cluster_configs", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (cIface *clusterConfigsClient) Update(clusterIdParam string, idsClusterConfigParam model.IdsClusterConfig) (model.IdsClusterConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.cluster_configs", "update", inputDataValue, executionContext)
	var emptyOutput model.IdsClusterConfig
	if methodResult.IsSuccess() {
