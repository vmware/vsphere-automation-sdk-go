...
func NewIdsEventsClient(connector client.Connector) *idsEventsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_events")
...
func (iIface *idsEventsClient) Create(policyIdsEventDataRequestParam model.PolicyIdsEventDataRequest) (model.PolicyIdsEventsBySignatureResult, error) {
	...
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_events", "create", inputDataValue, executionContext)
	var emptyOutput model.PolicyIdsEventsBySignatureResult
...
func (iIface *idsEventsClient) Get(idsEventIdParam string) (model.PolicyIdsEventDetails, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_events", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyIdsEventDetails
	if methodResult.IsSuccess() {
