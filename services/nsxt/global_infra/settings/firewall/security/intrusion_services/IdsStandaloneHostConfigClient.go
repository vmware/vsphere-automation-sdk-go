...
func NewIdsStandaloneHostConfigClient(connector client.Connector) *idsStandaloneHostConfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_standalone_host_config")
...
func (iIface *idsStandaloneHostConfigClient) Get() (model.IdsStandaloneHostConfig, error) {
	...
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_standalone_host_config", "get", inputDataValue, executionContext)
	var emptyOutput model.IdsStandaloneHostConfig
...
func (iIface *idsStandaloneHostConfigClient) Patch(idsStandaloneHostConfigParam model.IdsStandaloneHostConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_standalone_host_config", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), iIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (iIface *idsStandaloneHostConfigClient) Update(idsStandaloneHostConfigParam model.IdsStandaloneHostConfig) (model.IdsStandaloneHostConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	iIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := iIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.ids_standalone_host_config", "update", inputDataValue, executionContext)
	var emptyOutput model.IdsStandaloneHostConfig
	if methodResult.IsSuccess() {
