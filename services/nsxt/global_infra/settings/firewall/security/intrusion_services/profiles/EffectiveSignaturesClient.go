...
func NewEffectiveSignaturesClient(connector vapiProtocolClient_.Connector) *effectiveSignaturesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.profiles.effective_signatures")
...
func (eIface *effectiveSignaturesClient) Get(profileIdParam string, enforcementPointPathParam *string) (nsx_policyModel.IdsProfileSignatures, error) {
	...
	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security.intrusion_services.profiles.effective_signatures", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.IdsProfileSignatures
