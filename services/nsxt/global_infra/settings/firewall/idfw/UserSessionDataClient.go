...
func NewUserSessionDataClient(connector vapiProtocolClient_.Connector) *userSessionDataClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.user_session_data")
...
func (uIface *userSessionDataClient) Get(enforcementPointPathParam *string) (nsx_policyModel.IdfwUserSessionDataAndMappings, error) {
	...
	methodResult := uIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.user_session_data", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.IdfwUserSessionDataAndMappings
