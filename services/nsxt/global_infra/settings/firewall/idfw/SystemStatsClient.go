...
func NewSystemStatsClient(connector vapiProtocolClient_.Connector) *systemStatsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.system_stats")
...
func (sIface *systemStatsClient) Get(enforcementPointPathParam *string) (nsx_policyModel.IdfwSystemStats, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.system_stats", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.IdfwSystemStats
