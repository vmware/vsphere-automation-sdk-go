...
/*
 * Client stubs for service: VmStats
 * Functions that implement the generated VmStatsClient interface
 */
...
type DefaultVmStatsClient struct {
...
func NewDefaultVmStatsClient(connector client.Connector) *DefaultVmStatsClient {
	interfaceName := "com.vmware.nsx_policy.global_infra.settings.firewall.idfw.vm_stats"
	...
	vIface := DefaultVmStatsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	vIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	vIface.methodNameToDefMap["get"] = vIface.getMethodDefinition()
	return &vIface
}

func (vIface *DefaultVmStatsClient) Get(vmIdParam string, enforcementPointPathParam *string) (model.IdfwVmStats, error) {
...
func (vIface *DefaultVmStatsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := vIface.connector.GetApiProvider().Invoke(vIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (vIface *DefaultVmStatsClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(vIface.interfaceName)
	typeConverter := vIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(vmStatsGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(vmStatsGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmStatsClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmStatsClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmStatsClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmStatsClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmStatsClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmStatsClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultVmStatsClient.get method's errors.NotFound error - %s",
