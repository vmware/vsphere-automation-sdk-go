...
func NewVmStatsClient(connector vapiProtocolClient_.Connector) *vmStatsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.vm_stats")
...
func (vIface *vmStatsClient) Get(vmIdParam string, enforcementPointPathParam *string) (nsx_policyModel.IdfwVmStats, error) {
	...
	methodResult := vIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.vm_stats", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.IdfwVmStats
