...
/*
 * Client stubs for service: UserSessionData
 * Functions that implement the generated UserSessionDataClient interface
 */
...
type DefaultUserSessionDataClient struct {
...
func NewDefaultUserSessionDataClient(connector client.Connector) *DefaultUserSessionDataClient {
	interfaceName := "com.vmware.nsx_policy.global_infra.settings.firewall.idfw.user_session_data"
	...
	uIface := DefaultUserSessionDataClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	uIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	uIface.methodNameToDefMap["get"] = uIface.getMethodDefinition()
	return &uIface
}

func (uIface *DefaultUserSessionDataClient) Get(enforcementPointPathParam *string) (model.IdfwUserSessionDataAndMappings, error) {
...
func (uIface *DefaultUserSessionDataClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := uIface.connector.GetApiProvider().Invoke(uIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (uIface *DefaultUserSessionDataClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(uIface.interfaceName)
	typeConverter := uIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(userSessionDataGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(userSessionDataGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUserSessionDataClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUserSessionDataClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUserSessionDataClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUserSessionDataClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUserSessionDataClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUserSessionDataClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultUserSessionDataClient.get method's errors.NotFound error - %s",
