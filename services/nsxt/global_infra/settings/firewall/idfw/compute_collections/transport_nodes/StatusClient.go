...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.compute_collections.transport_nodes.status")
...
func (sIface *statusClient) List(computeCollectionIdParam string, enforcementPointPathParam *string) (model.IdfwTransportNodeStatusListResult, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.compute_collections.transport_nodes.status", "list", inputDataValue, executionContext)
	var emptyOutput model.IdfwTransportNodeStatusListResult
