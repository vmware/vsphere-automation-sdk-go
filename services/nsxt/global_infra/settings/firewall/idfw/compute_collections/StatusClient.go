...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.compute_collections.status")
...
func (sIface *statusClient) Get(computeCollectionIdParam string, enforcementPointPathParam *string) (model.IdfwComputeCollectionStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.compute_collections.status", "get", inputDataValue, executionContext)
	var emptyOutput model.IdfwComputeCollectionStatus
...
func (sIface *statusClient) List(enforcementPointPathParam *string) (model.IdfwComputeCollectionListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.compute_collections.status", "list", inputDataValue, executionContext)
	var emptyOutput model.IdfwComputeCollectionListResult
	if methodResult.IsSuccess() {
