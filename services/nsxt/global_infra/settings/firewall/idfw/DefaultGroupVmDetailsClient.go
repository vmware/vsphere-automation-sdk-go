...
/*
 * Client stubs for service: GroupVmDetails
 * Functions that implement the generated GroupVmDetailsClient interface
 */
...
type DefaultGroupVmDetailsClient struct {
...
func NewDefaultGroupVmDetailsClient(connector client.Connector) *DefaultGroupVmDetailsClient {
	interfaceName := "com.vmware.nsx_policy.global_infra.settings.firewall.idfw.group_vm_details"
	...
	gIface := DefaultGroupVmDetailsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (gIface *DefaultGroupVmDetailsClient) List(groupPathParam string, enforcementPointPathParam *string) (model.PolicyIdfwGroupVmDetailListResult, error) {
...
func (gIface *DefaultGroupVmDetailsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := gIface.connector.GetApiProvider().Invoke(gIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (gIface *DefaultGroupVmDetailsClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(gIface.interfaceName)
	typeConverter := gIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(groupVmDetailsListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(groupVmDetailsListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupVmDetailsClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupVmDetailsClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupVmDetailsClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupVmDetailsClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupVmDetailsClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupVmDetailsClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultGroupVmDetailsClient.list method's errors.NotFound error - %s",
