...
func NewUserStatsClient(connector client.Connector) *userStatsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.user_stats")
...
func (uIface *userStatsClient) Get(userIdParam string, enforcementPointPathParam *string) (model.IdfwUserStats, error) {
	...
	methodResult := uIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.idfw.user_stats", "get", inputDataValue, executionContext)
	var emptyOutput model.IdfwUserStats
