...
func NewSecurityClient(connector client.Connector) *securityClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.settings.firewall.security")
...
func (sIface *securityClient) Get() (model.DfwFirewallConfiguration, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security", "get", inputDataValue, executionContext)
	var emptyOutput model.DfwFirewallConfiguration
...
func (sIface *securityClient) Patch(dfwFirewallConfigurationParam model.DfwFirewallConfiguration) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (sIface *securityClient) Update(dfwFirewallConfigurationParam model.DfwFirewallConfiguration) (model.DfwFirewallConfiguration, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.settings.firewall.security", "update", inputDataValue, executionContext)
	var emptyOutput model.DfwFirewallConfiguration
	if methodResult.IsSuccess() {
