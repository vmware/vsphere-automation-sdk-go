...
func (dIface *domainsClient) Delete(domainIdParam string) error {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains", "delete", inputDataValue, executionContext)
...
func (dIface *domainsClient) Patch(domainIdParam string, domainParam model.Domain) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), dIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (dIface *domainsClient) Update(domainIdParam string, domainParam model.Domain) (model.Domain, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	dIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains", "update", inputDataValue, executionContext)
	var emptyOutput model.Domain
	if methodResult.IsSuccess() {
