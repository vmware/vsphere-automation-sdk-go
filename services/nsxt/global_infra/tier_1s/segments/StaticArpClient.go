...
func (sIface *staticArpClient) Delete(tier1IdParam string, segmentIdParam string) error {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_1s.segments.static_arp", "delete", inputDataValue, executionContext)
...
func (sIface *staticArpClient) Patch(tier1IdParam string, segmentIdParam string, staticARPConfigParam model.StaticARPConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_1s.segments.static_arp", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (sIface *staticArpClient) Update(tier1IdParam string, segmentIdParam string, staticARPConfigParam model.StaticARPConfig) (model.StaticARPConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_1s.segments.static_arp", "update", inputDataValue, executionContext)
	var emptyOutput model.StaticARPConfig
	if methodResult.IsSuccess() {
