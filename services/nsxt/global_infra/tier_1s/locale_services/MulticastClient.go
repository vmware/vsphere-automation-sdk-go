...
func NewMulticastClient(connector client.Connector) *multicastClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.tier_1s.locale_services.multicast")
...
func (mIface *multicastClient) Get(tier1IdParam string, localeServicesIdParam string) (model.PolicyTier1MulticastConfig, error) {
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_1s.locale_services.multicast", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyTier1MulticastConfig
...
func (mIface *multicastClient) Patch(tier1IdParam string, localeServicesIdParam string, policyTier1MulticastConfigParam model.PolicyTier1MulticastConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	mIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_1s.locale_services.multicast", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), mIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (mIface *multicastClient) Update(tier1IdParam string, localeServicesIdParam string, policyTier1MulticastConfigParam model.PolicyTier1MulticastConfig) (model.PolicyTier1MulticastConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	mIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.tier_1s.locale_services.multicast", "update", inputDataValue, executionContext)
	var emptyOutput model.PolicyTier1MulticastConfig
	if methodResult.IsSuccess() {
