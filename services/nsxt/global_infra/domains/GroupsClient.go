...
func (gIface *groupsClient) Delete(domainIdParam string, groupIdParam string, failIfSubtreeExistsParam *bool, forceParam *bool) error {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains.groups", "delete", inputDataValue, executionContext)
...
func (gIface *groupsClient) Patch(domainIdParam string, groupIdParam string, groupParam model.Group) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	gIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains.groups", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), gIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (gIface *groupsClient) Update(domainIdParam string, groupIdParam string, groupParam model.Group) (model.Group, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	gIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains.groups", "update", inputDataValue, executionContext)
	var emptyOutput model.Group
	if methodResult.IsSuccess() {
