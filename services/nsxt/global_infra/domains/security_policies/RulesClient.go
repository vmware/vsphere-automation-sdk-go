...
func (rIface *rulesClient) Delete(domainIdParam string, securityPolicyIdParam string, ruleIdParam string) error {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains.security_policies.rules", "delete", inputDataValue, executionContext)
...
func (rIface *rulesClient) Patch(domainIdParam string, securityPolicyIdParam string, ruleIdParam string, ruleParam model.Rule) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains.security_policies.rules", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), rIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (rIface *rulesClient) Revise(domainIdParam string, securityPolicyIdParam string, ruleIdParam string, ruleParam model.Rule, anchorPathParam *string, operationParam *string) (model.Rule, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains.security_policies.rules", "revise", inputDataValue, executionContext)
	var emptyOutput model.Rule
	if methodResult.IsSuccess() {
...
func (rIface *rulesClient) Update(domainIdParam string, securityPolicyIdParam string, ruleIdParam string, ruleParam model.Rule) (model.Rule, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.domains.security_policies.rules", "update", inputDataValue, executionContext)
	var emptyOutput model.Rule
	if methodResult.IsSuccess() {
