...
func NewLbAppProfilesClient(connector client.Connector) *lbAppProfilesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.lb_app_profiles")
...
func (lIface *lbAppProfilesClient) Delete(lbAppProfileIdParam string, forceParam *bool) error {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.lb_app_profiles", "delete", inputDataValue, executionContext)
...
func (lIface *lbAppProfilesClient) Get(lbAppProfileIdParam string) (*data.StructValue, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	lIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.lb_app_profiles", "get", inputDataValue, executionContext)
	var emptyOutput *data.StructValue
	if methodResult.IsSuccess() {
...
func (lIface *lbAppProfilesClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.LBAppProfileListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	lIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.lb_app_profiles", "list", inputDataValue, executionContext)
	var emptyOutput model.LBAppProfileListResult
	if methodResult.IsSuccess() {
...
func (lIface *lbAppProfilesClient) Patch(lbAppProfileIdParam string, lbAppProfileParam *data.StructValue) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	lIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.lb_app_profiles", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), lIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (lIface *lbAppProfilesClient) Update(lbAppProfileIdParam string, lbAppProfileParam *data.StructValue) (*data.StructValue, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	lIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.lb_app_profiles", "update", inputDataValue, executionContext)
	var emptyOutput *data.StructValue
	if methodResult.IsSuccess() {
