...
func NewReactionsClient(connector client.Connector) *reactionsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.reactions")
...
func (rIface *reactionsClient) Delete(reactionIdParam string) error {
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.reactions", "delete", inputDataValue, executionContext)
...
func (rIface *reactionsClient) Get(reactionIdParam string) (model.Reaction, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.reactions", "get", inputDataValue, executionContext)
	var emptyOutput model.Reaction
	if methodResult.IsSuccess() {
...
func (rIface *reactionsClient) List(cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.ReactionListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.reactions", "list", inputDataValue, executionContext)
	var emptyOutput model.ReactionListResult
	if methodResult.IsSuccess() {
...
func (rIface *reactionsClient) Patch(reactionIdParam string, reactionParam model.Reaction) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.reactions", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), rIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (rIface *reactionsClient) Update(reactionIdParam string, reactionParam model.Reaction) (model.Reaction, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	rIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.reactions", "update", inputDataValue, executionContext)
	var emptyOutput model.Reaction
	if methodResult.IsSuccess() {
