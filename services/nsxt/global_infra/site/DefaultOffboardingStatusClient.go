...
/*
 * Client stubs for service: OffboardingStatus
 * Functions that implement the generated OffboardingStatusClient interface
 */
...
type DefaultOffboardingStatusClient struct {
...
func NewDefaultOffboardingStatusClient(connector client.Connector) *DefaultOffboardingStatusClient {
	interfaceName := "com.vmware.nsx_policy.global_infra.site.offboarding_status"
	...
	oIface := DefaultOffboardingStatusClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	oIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	oIface.methodNameToDefMap["get"] = oIface.getMethodDefinition()
	return &oIface
}

func (oIface *DefaultOffboardingStatusClient) Get() (model.SiteOffBoardingState, error) {
...
func (oIface *DefaultOffboardingStatusClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := oIface.connector.GetApiProvider().Invoke(oIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (oIface *DefaultOffboardingStatusClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(oIface.interfaceName)
	typeConverter := oIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(offboardingStatusGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(offboardingStatusGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOffboardingStatusClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOffboardingStatusClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOffboardingStatusClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOffboardingStatusClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOffboardingStatusClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOffboardingStatusClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultOffboardingStatusClient.get method's errors.NotFound error - %s",
