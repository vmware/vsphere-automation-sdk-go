...
/*
 * Client stubs for service: ServicePaths
 * Functions that implement the generated ServicePathsClient interface
 */
...
type DefaultServicePathsClient struct {
...
func NewDefaultServicePathsClient(connector client.Connector) *DefaultServicePathsClient {
	interfaceName := "com.vmware.nsx_policy.global_infra.service_chains.service_paths"
	...
	sIface := DefaultServicePathsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	sIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	sIface.methodNameToDefMap["list"] = sIface.listMethodDefinition()
	return &sIface
}

func (sIface *DefaultServicePathsClient) List(serviceChainIdParam string, enforcementPointPathParam *string) (model.ServicePathListResult, error) {
...
func (sIface *DefaultServicePathsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := sIface.connector.GetApiProvider().Invoke(sIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (sIface *DefaultServicePathsClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(sIface.interfaceName)
	typeConverter := sIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(servicePathsListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(servicePathsListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultServicePathsClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultServicePathsClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultServicePathsClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultServicePathsClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultServicePathsClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultServicePathsClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultServicePathsClient.list method's errors.NotFound error - %s",
