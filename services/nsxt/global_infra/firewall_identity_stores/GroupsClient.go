...
type GroupsClient interface {
	...
	// @param filterValueParam Search filter value is a substring match pattern only which the target entity contains. It is case insentive and doesn't allow wildcard. (required)
...
func NewGroupsClient(connector vapiProtocolClient_.Connector) *groupsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.global_infra.firewall_identity_stores.groups")
...
func (gIface *groupsClient) List(firewallIdentityStoreIdParam string, filterValueParam string, cursorParam *string, enforcementPointPathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.DirectoryGroupListResults, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.global_infra.firewall_identity_stores.groups", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.DirectoryGroupListResults
