...
func NewSecurityClient(connector client.Connector) *securityClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra.settings.firewall.security")
...
func (sIface *securityClient) Get(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string) (model.DfwFirewallConfiguration, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra.settings.firewall.security", "get", inputDataValue, executionContext)
	var emptyOutput model.DfwFirewallConfiguration
...
func (sIface *securityClient) Patch(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, dfwFirewallConfigurationParam model.DfwFirewallConfiguration) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra.settings.firewall.security", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (sIface *securityClient) Update(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, dfwFirewallConfigurationParam model.DfwFirewallConfiguration) (model.DfwFirewallConfiguration, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra.settings.firewall.security", "update", inputDataValue, executionContext)
	var emptyOutput model.DfwFirewallConfiguration
	if methodResult.IsSuccess() {
