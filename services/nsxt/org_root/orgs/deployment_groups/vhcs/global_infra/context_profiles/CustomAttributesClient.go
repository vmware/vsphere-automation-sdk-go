...
type CustomAttributesClient interface {
	...
	Create(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, policyAttributesParam model.PolicyAttributes, actionParam string) error
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Patch(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, policyAttributesParam model.PolicyAttributes) error
}
...
func NewCustomAttributesClient(connector client.Connector) *customAttributesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra.context_profiles.custom_attributes")
...
func (cIface *customAttributesClient) Create(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, policyAttributesParam model.PolicyAttributes, actionParam string) error {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra.context_profiles.custom_attributes", "create", inputDataValue, executionContext)
...
func (cIface *customAttributesClient) Patch(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, policyAttributesParam model.PolicyAttributes) error {
	typeConverter := cIface.connector.TypeConverter()
	executionContext := cIface.connector.NewExecutionContext()
	...
	connectionMetadata["isStreamingResponse"] = false
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra.context_profiles.custom_attributes", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.GetErrorBindingType(methodResult.Error().Name()))
