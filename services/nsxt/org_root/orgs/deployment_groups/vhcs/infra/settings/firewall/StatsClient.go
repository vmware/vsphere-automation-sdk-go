...
func NewStatsClient(connector client.Connector) *statsClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.settings.firewall.stats")
...
func (sIface *statsClient) Reset(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, categoryParam string, containerClusterPathParam *string, enforcementPointPathParam *string) error {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.settings.firewall.stats", "reset", inputDataValue, executionContext)
