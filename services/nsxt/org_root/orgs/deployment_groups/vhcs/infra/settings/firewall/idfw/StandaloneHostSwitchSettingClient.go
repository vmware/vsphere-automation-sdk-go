...
func NewStandaloneHostSwitchSettingClient(connector client.Connector) *standaloneHostSwitchSettingClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.settings.firewall.idfw.standalone_host_switch_setting")
...
func (sIface *standaloneHostSwitchSettingClient) Get(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string) (model.StandaloneHostIdfwConfiguration, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.settings.firewall.idfw.standalone_host_switch_setting", "get", inputDataValue, executionContext)
	var emptyOutput model.StandaloneHostIdfwConfiguration
...
func (sIface *standaloneHostSwitchSettingClient) Patch(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, standaloneHostIdfwConfigurationParam model.StandaloneHostIdfwConfiguration) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.settings.firewall.idfw.standalone_host_switch_setting", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), sIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (sIface *standaloneHostSwitchSettingClient) Update(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, standaloneHostIdfwConfigurationParam model.StandaloneHostIdfwConfiguration) (model.StandaloneHostIdfwConfiguration, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.settings.firewall.idfw.standalone_host_switch_setting", "update", inputDataValue, executionContext)
	var emptyOutput model.StandaloneHostIdfwConfiguration
	if methodResult.IsSuccess() {
