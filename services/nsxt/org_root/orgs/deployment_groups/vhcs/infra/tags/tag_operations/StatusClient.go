...
func NewStatusClient(connector client.Connector) *statusClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.tags.tag_operations.status")
...
func (sIface *statusClient) Get(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, operationIdParam string) (model.TagBulkOperationStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.tags.tag_operations.status", "get", inputDataValue, executionContext)
	var emptyOutput model.TagBulkOperationStatus
