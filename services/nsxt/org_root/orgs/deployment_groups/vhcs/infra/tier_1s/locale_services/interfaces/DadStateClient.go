...
func NewDadStateClient(connector client.Connector) *dadStateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.tier_1s.locale_services.interfaces.dad_state")
...
func (dIface *dadStateClient) Get(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, tier1IdParam string, localeServiceIdParam string, interfaceIdParam string, enforcementPointPathParam *string) (model.InterfaceDADState, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.tier_1s.locale_services.interfaces.dad_state", "get", inputDataValue, executionContext)
	var emptyOutput model.InterfaceDADState
