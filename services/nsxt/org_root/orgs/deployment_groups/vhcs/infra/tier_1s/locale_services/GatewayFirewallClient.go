...
func NewGatewayFirewallClient(connector client.Connector) *gatewayFirewallClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.tier_1s.locale_services.gateway_firewall")
...
func (gIface *gatewayFirewallClient) List(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, tier1IdParam string, localeServicesIdParam string) (model.GatewayPolicyListResult, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.tier_1s.locale_services.gateway_firewall", "list", inputDataValue, executionContext)
	var emptyOutput model.GatewayPolicyListResult
