...
func NewStateClient(connector client.Connector) *stateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.segments.dhcp_static_bindings.state")
...
func (sIface *stateClient) Get(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, segmentIdParam string, bindingIdParam string) (model.DhcpStaticBindingState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.infra.segments.dhcp_static_bindings.state", "get", inputDataValue, executionContext)
	var emptyOutput model.DhcpStaticBindingState
