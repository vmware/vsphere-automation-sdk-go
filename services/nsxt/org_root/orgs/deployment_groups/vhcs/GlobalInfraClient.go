...
func NewGlobalInfraClient(connector client.Connector) *globalInfraClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra")
...
func (gIface *globalInfraClient) Get(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, basePathParam *string, filterParam *string, typeFilterParam *string) (model.Infra, error) {
	...
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra", "get", inputDataValue, executionContext)
	var emptyOutput model.Infra
...
func (gIface *globalInfraClient) Patch(orgIdParam string, deploymentGroupIdParam string, vhcIdParam string, infraParam model.Infra, enforceRevisionCheckParam *bool) error {
	typeConverter := gIface.connector.TypeConverter()
	executionContext := gIface.connector.NewExecutionContext()
	...
	connectionMetadata["isStreamingResponse"] = false
	gIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := gIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.org_root.orgs.deployment_groups.vhcs.global_infra", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), gIface.GetErrorBindingType(methodResult.Error().Name()))
