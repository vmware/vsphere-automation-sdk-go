...
type LbNodeCapacityStatusClient interface {
	...
	// API is used to retrieve available load balancer capacity status for all edge or virtual network appliance clusters shared with Project. If the parameter ?service_cluster_path=<service-cluster-path> exists, then API only shows the available load balancer capacity status for this specific edge or virtual network appliance cluster. VCF Load Balancer availability in terms of use-cases and editions is specified in the VMware Cloud Foundation Feature Comparison and Upgrade Paths Guide. Please review before consuming these APIs.
	//
