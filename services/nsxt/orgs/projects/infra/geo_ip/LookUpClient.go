...
type LookUpClient interface {
	...
	Get(orgIdParam string, projectIdParam string, ipAddressParam string) (nsx_policyModel.GeoIpLookUp, error)
}
...
func NewLookUpClient(connector vapiProtocolClient_.Connector) *lookUpClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.geo_ip.look_up")
...
func (lIface *lookUpClient) Get(orgIdParam string, projectIdParam string, ipAddressParam string) (nsx_policyModel.GeoIpLookUp, error) {
	...
	methodResult := lIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.geo_ip.look_up", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.GeoIpLookUp
