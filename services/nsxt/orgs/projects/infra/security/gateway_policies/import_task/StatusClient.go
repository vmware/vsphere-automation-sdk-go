...
type StatusClient interface {
	...
	// Get the status of the latest import task.
	//
...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.infra.security.gateway_policies.import_task.status")
...
func (sIface *statusClient) Get(orgIdParam string, projectIdParam string) (nsx_policyModel.ImportTask, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.infra.security.gateway_policies.import_task.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.ImportTask
