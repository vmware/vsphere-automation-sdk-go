...
func NewStateClient(connector vapiProtocolClient_.Connector) *stateClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.vpcs.limits.state")
...
func (sIface *stateClient) List(orgIdParam string, projectIdParam string, vpcIdParam string) (nsx_policyModel.LimitStateListResult, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.vpcs.limits.state", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.LimitStateListResult
