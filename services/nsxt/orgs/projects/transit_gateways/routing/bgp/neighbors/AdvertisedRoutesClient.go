...
type AdvertisedRoutesClient interface {
	...
	// Returns routes advertised by the BGP neighbor from all edge nodes on which this neighbor is currently enabled.
	//
...
func NewAdvertisedRoutesClient(connector vapiProtocolClient_.Connector) *advertisedRoutesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.orgs.projects.transit_gateways.routing.bgp.neighbors.advertised_routes")
...
func (aIface *advertisedRoutesClient) List(orgIdParam string, projectIdParam string, transitGatewayIdParam string, neighborIdParam string, countParam *int64, cursorParam *string, enforcementPointPathParam *string, includedFieldsParam *string, neighborAddressParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.BgpNeighborRoutesListResult, error) {
	...
	methodResult := aIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.orgs.projects.transit_gateways.routing.bgp.neighbors.advertised_routes", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.BgpNeighborRoutesListResult
