...
type SecurityConfigClient interface {
	...
	// @param featureParam Feature to be activated/deactivated per centralized transit gateway. GFW - Gateway Firewall IDFW - Identity Firewall IDPS - Intrusion Detection System GEOIP_MONITORING - Geo IP Monitoring TGWs under custom project will support only GFW and GEOIP_MONITORING Features. TGWs under default project will support all the security features listed above. GFW must be enabled to turn on other security features. (optional)
