...
func NewServiceStateClient(connector client.Connector) *serviceStateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.settings.firewall.security.intrusion_services.service_state")
...
func (sIface *serviceStateClient) Get() (model.PolicyIdsServiceState, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.settings.firewall.security.intrusion_services.service_state", "get", inputDataValue, executionContext)
	var emptyOutput model.PolicyIdsServiceState
