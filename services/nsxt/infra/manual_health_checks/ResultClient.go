...
type ResultClient interface {
	...
	// @return com.vmware.nsx_policy.model.PolicyManualHealthCheckResult
	//
	...
	Get(manualHealthCheckIdParam string) (nsx_policyModel.PolicyManualHealthCheckResult, error)
}
...
func NewResultClient(connector vapiProtocolClient_.Connector) *resultClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.manual_health_checks.result")
...
func (rIface *resultClient) Get(manualHealthCheckIdParam string) (nsx_policyModel.PolicyManualHealthCheckResult, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PolicyManualHealthCheckResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.manual_health_checks.result", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyManualHealthCheckResult
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PolicyManualHealthCheckResult), nil
	} else {
