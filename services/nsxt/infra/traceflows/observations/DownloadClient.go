...
func NewDownloadClient(connector vapiProtocolClient_.Connector) *downloadClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.traceflows.observations.download")
...
func (dIface *downloadClient) List(traceflowIdParam string, enforcementPointPathParam *string) (nsx_policyModel.TraceflowObservationListResult, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.traceflows.observations.download", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.TraceflowObservationListResult
