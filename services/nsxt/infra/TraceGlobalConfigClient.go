...
func NewTraceGlobalConfigClient(connector client.Connector) *traceGlobalConfigClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.trace_global_config")
...
func (tIface *traceGlobalConfigClient) Get() (model.TraceWorkflowGlobalConfig, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.trace_global_config", "get", inputDataValue, executionContext)
	var emptyOutput model.TraceWorkflowGlobalConfig
...
func (tIface *traceGlobalConfigClient) Patch(traceWorkflowGlobalConfigParam model.TraceWorkflowGlobalConfig) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.trace_global_config", "patch", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), tIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (tIface *traceGlobalConfigClient) Update(traceWorkflowGlobalConfigParam model.TraceWorkflowGlobalConfig) (model.TraceWorkflowGlobalConfig, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.trace_global_config", "update", inputDataValue, executionContext)
	var emptyOutput model.TraceWorkflowGlobalConfig
	if methodResult.IsSuccess() {
