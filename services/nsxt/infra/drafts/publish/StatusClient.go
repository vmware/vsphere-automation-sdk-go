...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.drafts.publish.status")
...
func (sIface *statusClient) Get(draftIdParam string) (nsx_policyModel.PublishTask, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.drafts.publish.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PublishTask
