...
func NewCancelClient(connector vapiProtocolClient_.Connector) *cancelClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.security.gateway_policies.export.cancel")
...
func (cIface *cancelClient) Create(categoryParam *string, draftPathParam *string, scopePathParam *string) (nsx_policyModel.FirewallExportTask, error) {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.security.gateway_policies.export.cancel", "create", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.FirewallExportTask
