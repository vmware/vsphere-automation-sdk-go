...
type CancelClient interface {
	...
	Create() (nsx_policyModel.ImportTask, error)
}
...
func NewCancelClient(connector vapiProtocolClient_.Connector) *cancelClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.security.gateway_policies.import_task.cancel")
...
func (cIface *cancelClient) Create() (nsx_policyModel.ImportTask, error) {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.security.gateway_policies.import_task.cancel", "create", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.ImportTask
