...
type DependentsClient interface {
	...
	Get(draftIdParam string) (nsx_policyModel.DependentDetailsResponse, error)
}
...
func NewDependentsClient(connector vapiProtocolClient_.Connector) *dependentsClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.security.gateway_policies.drafts.aggregated.dependents")
...
func (dIface *dependentsClient) Get(draftIdParam string) (nsx_policyModel.DependentDetailsResponse, error) {
	...
	methodResult := dIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.security.gateway_policies.drafts.aggregated.dependents", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.DependentDetailsResponse
