...
func NewCancelClient(connector vapiProtocolClient_.Connector) *cancelClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.security.gateway_policies.drafts.publish.cancel")
...
func (cIface *cancelClient) Create(draftIdParam string) (nsx_policyModel.PublishTask, error) {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.security.gateway_policies.drafts.publish.cancel", "create", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PublishTask
