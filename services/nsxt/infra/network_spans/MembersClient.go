...
type MembersClient interface {
	...
	// Read Network Span Members. This will list all the span members based on member type VcCluster or StandaloneHost. If no member_type is given, default member_type is VcCluster.
	//
...
func NewMembersClient(connector vapiProtocolClient_.Connector) *membersClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.network_spans.members")
...
func (mIface *membersClient) List(spanIdParam string, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, memberTypeParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.SpanMembersListResult, error) {
	...
	methodResult := mIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.network_spans.members", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.SpanMembersListResult
