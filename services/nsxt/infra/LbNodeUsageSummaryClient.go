...
type LbNodeUsageSummaryClient interface {
	...
	// The API is used to retrieve the load balancer node usage summary of all nodes for every enforcement point. - If the parameter ?include_usages=true exists, the property node_usages are included in response. By default, the property node_usages is not included in response. - If parameter ?enforcement_point_path=<enforcement-point-path> exists, only node usage summary from specific enforcement point is included in response. If no enforcement point path is specified, information will be aggregated from each enforcement point. - If parameter ?node_type=EdgeServiceNode exists, it will only return the LB node usage summary for edge service nodes. If parameter ?node_type=EdgeNode exists, it will only return the LB node usage summary for edge nodes. If the parameter is not specified, it will return LB node usage summary for both edge nodes and edge service nodes. NSX Load Balancer availability in terms of use-cases and editions is specified in NSX Feature and Edition Guide. Please review before consuming those APIs.
	//
