...
type RunbookInvocationsClient interface {
	...
	Create(odsRunbookInvocationFilterParam nsx_policyModel.OdsRunbookInvocationFilter, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.OdsRunbookInvocationListResult, error)
	...
	// @throws InternalServerError  Internal Server Error
	// @throws NotFound  Not Found
	Create0(invocationIdParam string, odsRunbookInvocationParam nsx_policyModel.OdsRunbookInvocation) error
...
func (rIface *runbookInvocationsClient) Create(odsRunbookInvocationFilterParam nsx_policyModel.OdsRunbookInvocationFilter, cursorParam *string, includeMarkForDeleteObjectsParam *bool, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.OdsRunbookInvocationListResult, error) {
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), RunbookInvocationsCreateOutputType())
...
func (rIface *runbookInvocationsClient) Create0(invocationIdParam string, odsRunbookInvocationParam nsx_policyModel.OdsRunbookInvocation) error {
	typeConverter := rIface.connector.TypeConverter()
	executionContext := rIface.connector.NewExecutionContext()
	operationRestMetaData := runbookInvocationsCreate0RestMetadata()
	executionContext.SetConnectionMetadata(vapiCore_.RESTMetadataKey, operationRestMetaData)
	executionContext.SetConnectionMetadata(vapiCore_.ResponseTypeKey, vapiCore_.NewResponseType(true, false))

	sv := vapiBindings_.NewStructValueBuilder(runbookInvocationsCreate0InputType(), typeConverter)
	...
	methodResult := rIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sha.runbook_invocations", "create_0", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), rIface.GetErrorBindingType(methodResult.Error().Name()))
