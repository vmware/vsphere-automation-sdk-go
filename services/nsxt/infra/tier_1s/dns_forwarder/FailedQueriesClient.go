...
func NewFailedQueriesClient(connector vapiProtocolClient_.Connector) *failedQueriesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_1s.dns_forwarder.failed_queries")
...
func (fIface *failedQueriesClient) Get(tier1IdParam string, countParam *int64, enforcementPointPathParam *string) (nsx_policyModel.PolicyDnsFailedQueries, error) {
	...
	methodResult := fIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_1s.dns_forwarder.failed_queries", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyDnsFailedQueries
