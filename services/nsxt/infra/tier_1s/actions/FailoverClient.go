...
type FailoverClient interface {
	...
	Create(tier1IdParam string, gatewayFailoverRequestParametersParam nsx_policyModel.GatewayFailoverRequestParameters) error
}
...
func NewFailoverClient(connector vapiProtocolClient_.Connector) *failoverClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.tier_1s.actions.failover")
...
func (fIface *failoverClient) Create(tier1IdParam string, gatewayFailoverRequestParametersParam nsx_policyModel.GatewayFailoverRequestParameters) error {
	...
	methodResult := fIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.tier_1s.actions.failover", "create", inputDataValue, executionContext)
