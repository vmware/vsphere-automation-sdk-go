...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.enforcement_points.service_clusters.allocation.status")
...
func (sIface *statusClient) Get(siteIdParam string, enforcementpointIdParam string, serviceClusterIdParam string) (nsx_policyModel.PolicyServiceClusterAllocationStatus, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.PolicyServiceClusterAllocationStatus
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sites.enforcement_points.service_clusters.allocation.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyServiceClusterAllocationStatus
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.PolicyServiceClusterAllocationStatus), nil
	} else {
