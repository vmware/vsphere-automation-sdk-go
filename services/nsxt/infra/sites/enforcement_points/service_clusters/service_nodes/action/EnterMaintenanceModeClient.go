...
type EnterMaintenanceModeClient interface {
    ...
    // Put service node into maintenance mode if there is no VIFs attached. When ServiceNode is in maintenance mode, no configuration changes are allowed.
    //
...
func NewEnterMaintenanceModeClient(connector vapiProtocolClient_.Connector) *enterMaintenanceModeClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.enforcement_points.service_clusters.service_nodes.action.enter_maintenance_mode")
...
func (eIface *enterMaintenanceModeClient) Create(siteIdParam string, enforcementpointIdParam string, serviceClusterIdParam string, serviceNodeIdParam string) error {
	...
	methodResult := eIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sites.enforcement_points.service_clusters.service_nodes.action.enter_maintenance_mode", "create", inputDataValue, executionContext)
