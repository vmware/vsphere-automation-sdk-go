...
func NewStatusClient(connector vapiProtocolClient_.Connector) *statusClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.sites.enforcement_points.edge_clusters.remote_tep_connectivity.status")
...
func (sIface *statusClient) Get(siteIdParam string, enforcementpointIdParam string, edgeClusterIdParam string) (nsx_policyModel.PolicyEdgeClusterRemoteTunnelConnectivityStatus, error) {
	...
	methodResult := sIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.sites.enforcement_points.edge_clusters.remote_tep_connectivity.status", "get", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.PolicyEdgeClusterRemoteTunnelConnectivityStatus
