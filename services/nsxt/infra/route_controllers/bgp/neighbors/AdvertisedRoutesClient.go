...
type AdvertisedRoutesClient interface {
	...
	// Returns routes advertised by BGP neighbor from all virtual network appliance nodes on which this neighbor is currently enabled in CSV format.
	//
	...
	// @return com.vmware.nsx_policy.model.RouteControllerBgpNeighborRoutesListResult
	//
...
func NewAdvertisedRoutesClient(connector vapiProtocolClient_.Connector) *advertisedRoutesClient {
	interfaceIdentifier := vapiCore_.NewInterfaceIdentifier("com.vmware.nsx_policy.infra.route_controllers.bgp.neighbors.advertised_routes")
...
func (aIface *advertisedRoutesClient) List(routeControllerIdParam string, neighborIdParam string, countParam *int64, cursorParam *string, enforcementPointPathParam *string, includedFieldsParam *string, neighborAddressParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (nsx_policyModel.RouteControllerBgpNeighborRoutesListResult, error) {
	...
	if inputError != nil {
		var emptyOutput nsx_policyModel.RouteControllerBgpNeighborRoutesListResult
		return emptyOutput, vapiBindings_.VAPIerrorsToError(inputError)
	}

	methodResult := aIface.connector.GetApiProvider().Invoke("com.vmware.nsx_policy.infra.route_controllers.bgp.neighbors.advertised_routes", "list", inputDataValue, executionContext)
	var emptyOutput nsx_policyModel.RouteControllerBgpNeighborRoutesListResult
	if methodResult.IsSuccess() {
		...
		return output.(nsx_policyModel.RouteControllerBgpNeighborRoutesListResult), nil
	} else {
