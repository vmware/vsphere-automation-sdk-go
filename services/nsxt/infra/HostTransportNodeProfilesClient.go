...
type HostTransportNodeProfilesClient interface {
	...
	// @param updateVcpConfigParam If transport node profile is attached to a VCP managed cluster, update the VCP cluster document with this transport node profile path. After a transport node profile is updated using this parameter, it no longer VCP managed. (optional, default to false)
	// @return com.vmware.nsx_policy.model.PolicyHostTransportNodeProfile
	//
	// @throws InvalidRequest  Bad Request, Precondition Failed
	// @throws TimedOut  Gateway Timeout
