...
type LocalLibraryClient interface {
    ...
    // @return Identifier of the newly created LibraryModel.
    ...
    // The parameter must be an identifier for the resource type: ``com.vmware.content.Library``.
    // @return The LibraryModel instance associated with ``library_id``.
    // @throws NotFound  if the library specified by ``library_id`` does not exist.
    // @throws InvalidElementType  if the library specified by ``library_id`` is not a local library.
    // @throws com.vmware.vapi.std.errors.Unauthorized if you do not have all of the privileges described as follows: 
    //
    ...
    // @throws NotFound  if the library specified by ``library_id`` does not exist.
    // @throws NotAllowedInCurrentState  if the library specified by ``library_id`` is a published library with JSON persistence enabled (see library.PublishInfo#persistJsonEnabled) and the content of the library has been deleted from the storage backings (see LibraryModel#storageBackings) associated with it.
    // @throws InvalidElementType  if the library specified by ``library_id`` is not a local library.
	...
	Publish(libraryIdParam string, subscriptionsParam []LocalLibraryDestinationSpec) error
}
