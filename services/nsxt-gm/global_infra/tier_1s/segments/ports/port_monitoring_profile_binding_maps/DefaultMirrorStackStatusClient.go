...
/*
 * Client stubs for service: MirrorStackStatus
 * Functions that implement the generated MirrorStackStatusClient interface
 */
...
type DefaultMirrorStackStatusClient struct {
...
func NewDefaultMirrorStackStatusClient(connector client.Connector) *DefaultMirrorStackStatusClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.tier_1s.segments.ports.port_monitoring_profile_binding_maps.mirror_stack_status"
	...
	mIface := DefaultMirrorStackStatusClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (mIface *DefaultMirrorStackStatusClient) List(tier1IdParam string, segmentIdParam string, portIdParam string, portMonitoringProfileBindingMapIdParam string, cursorParam *string, enforcementPointPathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.MirrorStackStatusListResult, error) {
...
func (mIface *DefaultMirrorStackStatusClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := mIface.connector.GetApiProvider().Invoke(mIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (mIface *DefaultMirrorStackStatusClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(mIface.interfaceName)
	typeConverter := mIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(mirrorStackStatusListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(mirrorStackStatusListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMirrorStackStatusClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMirrorStackStatusClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMirrorStackStatusClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMirrorStackStatusClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMirrorStackStatusClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMirrorStackStatusClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMirrorStackStatusClient.list method's errors.NotFound error - %s",
