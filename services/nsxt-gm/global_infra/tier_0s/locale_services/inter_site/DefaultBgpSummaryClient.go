...
/*
 * Client stubs for service: BgpSummary
 * Functions that implement the generated BgpSummaryClient interface
 */
...
type DefaultBgpSummaryClient struct {
...
func NewDefaultBgpSummaryClient(connector client.Connector) *DefaultBgpSummaryClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.tier_0s.locale_services.inter_site.bgp_summary"
	...
	bIface := DefaultBgpSummaryClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	bIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	bIface.methodNameToDefMap["get"] = bIface.getMethodDefinition()
	return &bIface
}

func (bIface *DefaultBgpSummaryClient) Get(tier0IdParam string, localeServicesIdParam string, cursorParam *string, edgePathParam *string, includedFieldsParam *string, pageSizeParam *int64, sortAscendingParam *bool, sortByParam *string) (model.PolicyEdgeClusterInterSiteBgpSummary, error) {
	typeConverter := bIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(bIface.interfaceIdentifier, "get")
	sv := bindings.NewStructValueBuilder(bgpSummaryGetInputType(), typeConverter)
	...
	operationRestMetaData := bgpSummaryGetRestMetadata()
	...
	if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), bgpSummaryGetOutputType())
...
func (bIface *DefaultBgpSummaryClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := bIface.connector.GetApiProvider().Invoke(bIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (bIface *DefaultBgpSummaryClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(bIface.interfaceName)
	typeConverter := bIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(bgpSummaryGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(bgpSummaryGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBgpSummaryClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBgpSummaryClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBgpSummaryClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBgpSummaryClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBgpSummaryClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBgpSummaryClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBgpSummaryClient.get method's errors.NotFound error - %s",
