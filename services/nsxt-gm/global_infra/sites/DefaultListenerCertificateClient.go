...
/*
 * Client stubs for service: Listener_certificate
 * Functions that implement the generated Listener_certificateClient interface
 */
...
type DefaultListener_certificateClient struct {
...
func NewDefaultListener_certificateClient(connector client.Connector) *DefaultListener_certificateClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.sites.listener_certificate"
	...
	lIface := DefaultListener_certificateClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	lIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	lIface.methodNameToDefMap["get"] = lIface.getMethodDefinition()
	return &lIface
}

func (lIface *DefaultListener_certificateClient) Get(addressParam string, portParam int64) (model.TlsListenerCertificate, error) {
...
func (lIface *DefaultListener_certificateClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := lIface.connector.GetApiProvider().Invoke(lIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (lIface *DefaultListener_certificateClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(lIface.interfaceName)
	typeConverter := lIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(listenerCertificateGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(listenerCertificateGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultListener_certificateClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultListener_certificateClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultListener_certificateClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultListener_certificateClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultListener_certificateClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultListener_certificateClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultListener_certificateClient.get method's errors.NotFound error - %s",
