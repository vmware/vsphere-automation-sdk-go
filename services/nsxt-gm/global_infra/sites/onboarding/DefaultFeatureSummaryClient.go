...
/*
 * Client stubs for service: FeatureSummary
 * Functions that implement the generated FeatureSummaryClient interface
 */
...
type DefaultFeatureSummaryClient struct {
...
func NewDefaultFeatureSummaryClient(connector client.Connector) *DefaultFeatureSummaryClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.sites.onboarding.feature_summary"
	...
	fIface := DefaultFeatureSummaryClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	fIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	fIface.methodNameToDefMap["get"] = fIface.getMethodDefinition()
	return &fIface
}

func (fIface *DefaultFeatureSummaryClient) Get(siteIdParam string) (model.ConflictingEntityListResponse, error) {
...
func (fIface *DefaultFeatureSummaryClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := fIface.connector.GetApiProvider().Invoke(fIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (fIface *DefaultFeatureSummaryClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(fIface.interfaceName)
	typeConverter := fIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(featureSummaryGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(featureSummaryGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureSummaryClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureSummaryClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureSummaryClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureSummaryClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureSummaryClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureSummaryClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultFeatureSummaryClient.get method's errors.NotFound error - %s",
