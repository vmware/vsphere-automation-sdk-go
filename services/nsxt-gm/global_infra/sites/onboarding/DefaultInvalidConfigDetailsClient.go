...
/*
 * Client stubs for service: InvalidConfigDetails
 * Functions that implement the generated InvalidConfigDetailsClient interface
 */
...
type DefaultInvalidConfigDetailsClient struct {
...
func NewDefaultInvalidConfigDetailsClient(connector client.Connector) *DefaultInvalidConfigDetailsClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.sites.onboarding.invalid_config_details"
	...
	iIface := DefaultInvalidConfigDetailsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	iIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	iIface.methodNameToDefMap["get"] = iIface.getMethodDefinition()
	return &iIface
}

func (iIface *DefaultInvalidConfigDetailsClient) Get(siteIdParam string, featureParam string) (model.FederationInvalidConfigurationDetailsResponse, error) {
...
func (iIface *DefaultInvalidConfigDetailsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := iIface.connector.GetApiProvider().Invoke(iIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (iIface *DefaultInvalidConfigDetailsClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(iIface.interfaceName)
	typeConverter := iIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(invalidConfigDetailsGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(invalidConfigDetailsGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultInvalidConfigDetailsClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultInvalidConfigDetailsClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultInvalidConfigDetailsClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultInvalidConfigDetailsClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultInvalidConfigDetailsClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultInvalidConfigDetailsClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultInvalidConfigDetailsClient.get method's errors.NotFound error - %s",
