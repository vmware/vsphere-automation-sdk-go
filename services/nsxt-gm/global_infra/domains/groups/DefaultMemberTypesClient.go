...
/*
 * Client stubs for service: MemberTypes
 * Functions that implement the generated MemberTypesClient interface
 */
...
type DefaultMemberTypesClient struct {
...
func NewDefaultMemberTypesClient(connector client.Connector) *DefaultMemberTypesClient {
	interfaceName := "com.vmware.nsx_global_policy.global_infra.domains.groups.member_types"
	...
	mIface := DefaultMemberTypesClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	mIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	mIface.methodNameToDefMap["get"] = mIface.getMethodDefinition()
	return &mIface
}

func (mIface *DefaultMemberTypesClient) Get(domainIdParam string, groupIdParam string) (model.GroupMemberTypeListResult, error) {
...
func (mIface *DefaultMemberTypesClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := mIface.connector.GetApiProvider().Invoke(mIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (mIface *DefaultMemberTypesClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(mIface.interfaceName)
	typeConverter := mIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(memberTypesGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(memberTypesGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMemberTypesClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMemberTypesClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMemberTypesClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMemberTypesClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMemberTypesClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMemberTypesClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultMemberTypesClient.get method's errors.NotFound error - %s",
