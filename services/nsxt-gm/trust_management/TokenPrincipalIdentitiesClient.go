...
func NewTokenPrincipalIdentitiesClient(connector client.Connector) *tokenPrincipalIdentitiesClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.trust_management.token_principal_identities")
...
func (tIface *tokenPrincipalIdentitiesClient) Create(tokenBasedPrincipalIdentityParam model.TokenBasedPrincipalIdentity) (model.TokenBasedPrincipalIdentity, error) {
	...
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.trust_management.token_principal_identities", "create", inputDataValue, executionContext)
	var emptyOutput model.TokenBasedPrincipalIdentity
...
func (tIface *tokenPrincipalIdentitiesClient) Delete(principalIdentityIdParam string) error {
	...
	connectionMetadata["isStreamingResponse"] = false
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.trust_management.token_principal_identities", "delete", inputDataValue, executionContext)
	if methodResult.IsSuccess() {
		return nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), tIface.GetErrorBindingType(methodResult.Error().Name()))
...
func (tIface *tokenPrincipalIdentitiesClient) Get(principalIdentityIdParam string) (model.TokenBasedPrincipalIdentity, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.trust_management.token_principal_identities", "get", inputDataValue, executionContext)
	var emptyOutput model.TokenBasedPrincipalIdentity
	if methodResult.IsSuccess() {
...
func (tIface *tokenPrincipalIdentitiesClient) List() (model.TokenBasedPrincipalIdentityListResult, error) {
	...
	connectionMetadata["isStreamingResponse"] = false
	tIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult := tIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.trust_management.token_principal_identities", "list", inputDataValue, executionContext)
	var emptyOutput model.TokenBasedPrincipalIdentityListResult
	if methodResult.IsSuccess() {
