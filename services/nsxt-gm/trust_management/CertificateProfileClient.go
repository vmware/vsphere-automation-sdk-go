...
func NewCertificateProfileClient(connector client.Connector) *certificateProfileClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.trust_management.certificate_profile")
...
func (cIface *certificateProfileClient) Get(serviceTypeParam string) (model.CertificateProfile, error) {
	...
	methodResult := cIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.trust_management.certificate_profile", "get", inputDataValue, executionContext)
	var emptyOutput model.CertificateProfile
