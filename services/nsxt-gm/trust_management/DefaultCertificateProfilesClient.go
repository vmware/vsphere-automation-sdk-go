...
/*
 * Client stubs for service: CertificateProfiles
 * Functions that implement the generated CertificateProfilesClient interface
 */
...
type DefaultCertificateProfilesClient struct {
...
func NewDefaultCertificateProfilesClient(connector client.Connector) *DefaultCertificateProfilesClient {
	interfaceName := "com.vmware.nsx_global_policy.trust_management.certificate_profiles"
	...
	cIface := DefaultCertificateProfilesClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	cIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	cIface.methodNameToDefMap["list"] = cIface.listMethodDefinition()
	return &cIface
}

func (cIface *DefaultCertificateProfilesClient) List() (model.CertificateProfileListResult, error) {
...
func (cIface *DefaultCertificateProfilesClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := cIface.connector.GetApiProvider().Invoke(cIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (cIface *DefaultCertificateProfilesClient) listMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(cIface.interfaceName)
	typeConverter := cIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(certificateProfilesListInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(certificateProfilesListOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfilesClient.list method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfilesClient.list method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfilesClient.list method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfilesClient.list method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfilesClient.list method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfilesClient.list method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfilesClient.list method's errors.NotFound error - %s",
