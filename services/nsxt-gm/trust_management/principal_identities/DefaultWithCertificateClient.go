...
/*
 * Client stubs for service: WithCertificate
 * Functions that implement the generated WithCertificateClient interface
 */
...
type DefaultWithCertificateClient struct {
...
func NewDefaultWithCertificateClient(connector client.Connector) *DefaultWithCertificateClient {
	interfaceName := "com.vmware.nsx_global_policy.trust_management.principal_identities.with_certificate"
	...
	wIface := DefaultWithCertificateClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
...
func (wIface *DefaultWithCertificateClient) Create(principalIdentityWithCertificateParam model.PrincipalIdentityWithCertificate) (model.PrincipalIdentity, error) {
...
func (wIface *DefaultWithCertificateClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := wIface.connector.GetApiProvider().Invoke(wIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (wIface *DefaultWithCertificateClient) createMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(wIface.interfaceName)
	typeConverter := wIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(withCertificateCreateInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(withCertificateCreateOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultWithCertificateClient.create method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultWithCertificateClient.create method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultWithCertificateClient.create method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultWithCertificateClient.create method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultWithCertificateClient.create method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultWithCertificateClient.create method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultWithCertificateClient.create method's errors.NotFound error - %s",
