...
func NewWithCertificateClient(connector client.Connector) *withCertificateClient {
	interfaceIdentifier := core.NewInterfaceIdentifier("com.vmware.nsx_global_policy.trust_management.principal_identities.with_certificate")
...
func (wIface *withCertificateClient) Create(principalIdentityWithCertificateParam model.PrincipalIdentityWithCertificate) (model.PrincipalIdentity, error) {
	...
	methodResult := wIface.connector.GetApiProvider().Invoke("com.vmware.nsx_global_policy.trust_management.principal_identities.with_certificate", "create", inputDataValue, executionContext)
	var emptyOutput model.PrincipalIdentity
