...
/*
 * Client stubs for service: CertificateProfile
 * Functions that implement the generated CertificateProfileClient interface
 */
...
type DefaultCertificateProfileClient struct {
...
func NewDefaultCertificateProfileClient(connector client.Connector) *DefaultCertificateProfileClient {
	interfaceName := "com.vmware.nsx_global_policy.trust_management.certificate_profile"
	...
	cIface := DefaultCertificateProfileClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	cIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	cIface.methodNameToDefMap["get"] = cIface.getMethodDefinition()
	return &cIface
}

func (cIface *DefaultCertificateProfileClient) Get(serviceTypeParam string) (model.CertificateProfile, error) {
...
func (cIface *DefaultCertificateProfileClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := cIface.connector.GetApiProvider().Invoke(cIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (cIface *DefaultCertificateProfileClient) getMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(cIface.interfaceName)
	typeConverter := cIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(certificateProfileGetInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(certificateProfileGetOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfileClient.get method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfileClient.get method's output - %s",
	...
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfileClient.get method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfileClient.get method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfileClient.get method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfileClient.get method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultCertificateProfileClient.get method's errors.NotFound error - %s",
