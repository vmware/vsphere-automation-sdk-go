...
/*
 * Client stubs for service: Backups
 * Functions that implement the generated BackupsClient interface
 */
...
type DefaultBackupsClient struct {
...
func NewDefaultBackupsClient(connector client.Connector) *DefaultBackupsClient {
	interfaceName := "com.vmware.nsx_global_policy.cluster.backups"
	interfaceIdentifier := core.NewInterfaceIdentifier(interfaceName)
	methodIdentifiers := []core.MethodIdentifier{
		core.NewMethodIdentifier(interfaceIdentifier, "retrievesshfingerprint"),
	}
	...
	bIface := DefaultBackupsClient{interfaceName: interfaceName, methodIdentifiers: methodIdentifiers, interfaceDefinition: interfaceDefinition, errorBindingMap: errorBindingMap, interfaceIdentifier: interfaceIdentifier, connector: connector}
	bIface.methodNameToDefMap = make(map[string]*core.MethodDefinition)
	bIface.methodNameToDefMap["retrievesshfingerprint"] = bIface.retrievesshfingerprintMethodDefinition()
	return &bIface
}

func (bIface *DefaultBackupsClient) Retrievesshfingerprint(remoteServerFingerprintRequestParam model.RemoteServerFingerprintRequest) (model.RemoteServerFingerprint, error) {
	typeConverter := bIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(bIface.interfaceIdentifier, "retrievesshfingerprint")
...
func (bIface *DefaultBackupsClient) Invoke(ctx *core.ExecutionContext, methodId core.MethodIdentifier, inputDataValue data.DataValue) core.MethodResult {
	methodResult := bIface.connector.GetApiProvider().Invoke(bIface.interfaceName, methodId.Name(), inputDataValue, ctx)
	return methodResult
}


func (bIface *DefaultBackupsClient) retrievesshfingerprintMethodDefinition() *core.MethodDefinition {
	interfaceIdentifier := core.NewInterfaceIdentifier(bIface.interfaceName)
	typeConverter := bIface.connector.TypeConverter()

	input, inputError := typeConverter.ConvertToDataDefinition(backupsRetrievesshfingerprintInputType())
	output, outputError := typeConverter.ConvertToDataDefinition(backupsRetrievesshfingerprintOutputType())
	if inputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBackupsClient.retrievesshfingerprint method's input - %s",
			bindings.VAPIerrorsToError(inputError).Error())
		return nil
	}
	if outputError != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBackupsClient.retrievesshfingerprint method's output - %s",
			bindings.VAPIerrorsToError(outputError).Error())
		return nil
	}
	methodIdentifier := core.NewMethodIdentifier(interfaceIdentifier, "retrievesshfingerprint")
	errorDefinitions := make([]data.ErrorDefinition, 0)
	bIface.errorBindingMap[errors.InvalidRequest{}.Error()] = errors.InvalidRequestBindingType()
	errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.InvalidRequestBindingType())
	if errError1 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBackupsClient.retrievesshfingerprint method's errors.InvalidRequest error - %s",
	...
	if errError2 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBackupsClient.retrievesshfingerprint method's errors.Unauthorized error - %s",
	...
	if errError3 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBackupsClient.retrievesshfingerprint method's errors.ServiceUnavailable error - %s",
	...
	if errError4 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBackupsClient.retrievesshfingerprint method's errors.InternalServerError error - %s",
	...
	if errError5 != nil {
		log.Errorf("Error in ConvertToDataDefinition for DefaultBackupsClient.retrievesshfingerprint method's errors.NotFound error - %s",
