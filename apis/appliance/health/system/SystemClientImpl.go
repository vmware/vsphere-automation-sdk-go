...
func (sIface *SystemClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(sIface.interfaceName)
}

func (sIface *SystemClientImpl) Definition() core.InterfaceDefinition {
    return sIface.interfaceDefinition
}

func (sIface *SystemClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (sIface *SystemClientImpl) Lastcheck() (time.Time, error) {
	...
	sv := bindings.NewStructValueBuilder(lastcheckInputType(), typeConverter)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput time.Time
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := lastcheckRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= sIface.Invoke(sIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput time.Time
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), lastcheckOutputType())
...
func (sIface *SystemClientImpl) Get() (HealthLevel, error) {
	typeConverter := sIface.connector.TypeConverter()
...
func (sIface *SystemClientImpl) lastcheckMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(lastcheckInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(lastcheckOutputType())
      if(inputError != nil) {
