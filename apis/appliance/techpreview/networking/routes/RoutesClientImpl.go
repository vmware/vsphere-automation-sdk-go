...
func (rIface *RoutesClientImpl) Test(gatewaysParam []string) (TestStatusInfo, error) {
...
func (rIface *RoutesClientImpl) Add(routeParam Route) error {
	typeConverter := rIface.connector.TypeConverter()
...
func (rIface *RoutesClientImpl) Set(routesParam []Route) error {
	typeConverter := rIface.connector.TypeConverter()
...
func (rIface *RoutesClientImpl) List() ([]RouteReadOnly, error) {
	typeConverter := rIface.connector.TypeConverter()
    ...
    if methodResult.IsSuccess() {
	    ...
	    return output.([]RouteReadOnly), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), rIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (rIface *RoutesClientImpl) Delete(routeParam Route) error {
	typeConverter := rIface.connector.TypeConverter()
