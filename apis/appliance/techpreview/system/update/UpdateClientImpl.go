...
func (uIface *UpdateClientImpl) Set(configParam UpdateStructSet) error {
...
func (uIface *UpdateClientImpl) Get() (UpdateStructGet, error) {
	typeConverter := uIface.connector.TypeConverter()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput UpdateStructGet
    ...
    if methodResult.IsSuccess() {
	    ...
	    return output.(UpdateStructGet), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), uIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
