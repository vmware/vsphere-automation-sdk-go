...
func (nIface *NetworkingClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(nIface.interfaceName)
}

func (nIface *NetworkingClientImpl) Definition() core.InterfaceDefinition {
    return nIface.interfaceDefinition
}

func (nIface *NetworkingClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (nIface *NetworkingClientImpl) Get() (Info, error) {
...
func (nIface *NetworkingClientImpl) Update(specParam UpdateSpec) error {
	typeConverter := nIface.connector.TypeConverter()
...
func (nIface *NetworkingClientImpl) Change(specParam ChangeSpec) error {
	typeConverter := nIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(nIface.interfaceIdentifier, "change")
	sv := bindings.NewStructValueBuilder(changeInputType(), typeConverter)
	sv.AddStructField("Spec", specParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
		return bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := changeRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	nIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= nIface.Invoke(nIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
    if methodResult.IsSuccess() {
...
func (nIface *NetworkingClientImpl) changeMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(changeInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(changeOutputType())
      if(inputError != nil) {
