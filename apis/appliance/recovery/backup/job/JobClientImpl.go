...
func (jIface *JobClientImpl) Cancel(idParam string) (ReturnResult, error) {
...
func (jIface *JobClientImpl) Create(pieceParam BackupRequest) (BackupJobStatus, error) {
	typeConverter := jIface.connector.TypeConverter()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput BackupJobStatus
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
    ...
    if methodResult.IsSuccess() {
	    ...
	    return output.(BackupJobStatus), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), jIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (jIface *JobClientImpl) Get(idParam string) (BackupJobStatus, error) {
	typeConverter := jIface.connector.TypeConverter()
