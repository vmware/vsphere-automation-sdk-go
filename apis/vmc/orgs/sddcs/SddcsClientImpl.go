...
func (sIface *SddcsClientImpl) Create(orgParam string, sddcConfigParam model.AwsSddcConfig, validateOnlyParam *bool) (model.Task, error) {
...
func (sIface *SddcsClientImpl) List(orgParam string, includeDeletedParam *bool) ([]model.Sddc, error) {
	typeConverter := sIface.connector.TypeConverter()
...
func (sIface *SddcsClientImpl) Patch(orgParam string, sddcParam string, sddcPatchRequestParam model.SddcPatchRequest) (model.Sddc, error) {
	...
	sv := bindings.NewStructValueBuilder(patchInputType(), typeConverter)
	sv.AddStructField("Org", orgParam)
	sv.AddStructField("Sddc", sddcParam)
	...
	operationRestMetaData := patchRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	sIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= sIface.Invoke(sIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput model.Sddc
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), patchOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(model.Sddc), nil
...
func (sIface *SddcsClientImpl) patchMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(patchInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(patchOutputType())
      if(inputError != nil) {
