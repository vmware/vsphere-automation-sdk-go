...
type FileClient interface {
    ...
    // Validates the files in the update session with the referenced identifier and ensures all necessary files are received. In the case where a file is missing, this method will return its name in the ValidationResult#missingFiles set. The user can add the missing files and try re-validating. For other type of errors, ValidationResult#invalidFiles will contain the list of invalid files.
    //
    ...
    Validate(updateSessionIdParam string) (ValidationResult, error) 
    ...
    // @return An Info class containing upload links as well as server side state tracking the transfer of the file.
    // @throws InvalidArgument  if the ``file_spec`` is invalid.
    // @throws NotFound  if the update session doesn't exist.
    // @throws Unauthorized  if the caller doesn't have ContentLibrary.ReadStorage privilege on the library item of the update session and source type SourceType#SourceType_PULL is requested for a file or datastore source endpoint (that is, not HTTP or HTTPs based endpoint).
    ...
    //
    // * Method execution requires ``System.Anonymous``.
    Add(updateSessionIdParam string, fileSpecParam AddSpec) (Info, error) 
    ...
    //
    // * Method execution requires ``System.Anonymous``.
    List(updateSessionIdParam string) ([]Info, error) 
    ...
    //
    // * Method execution requires ``System.Anonymous``.
    Get(updateSessionIdParam string, fileNameParam string) (Info, error) 
    ...
    //
    // @param sourceEndpointParam  the source endpoint to be probed.
    // @return the source endpoint probe result.
    // @throws com.vmware.vapi.std.errors.Unauthorized if you do not have all of the privileges described as follows: 
    //
    // * Method execution requires ``System.Anonymous``.
    Probe(sourceEndpointParam item.TransferEndpoint) (ProbeResult, error) 
