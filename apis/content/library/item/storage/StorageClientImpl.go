...
func (sIface *StorageClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(sIface.interfaceName)
}

func (sIface *StorageClientImpl) Definition() core.InterfaceDefinition {
    return sIface.interfaceDefinition
}

func (sIface *StorageClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (sIface *StorageClientImpl) Get(libraryItemIdParam string, fileNameParam string) ([]Info, error) {
...
func (sIface *StorageClientImpl) List(libraryItemIdParam string) ([]Info, error) {
	typeConverter := sIface.connector.TypeConverter()
...
func (sIface *StorageClientImpl) getMethodDefinition() *core.MethodDefinition {
	  ...
	  if(errError1 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for StorageClientImpl.get method's errors.NotFound error - %s",
...
func (sIface *StorageClientImpl) listMethodDefinition() *core.MethodDefinition {
	  ...
	  errDef1, errError1 := typeConverter.ConvertToDataDefinition(errors.NotFoundBindingType())
	  if(errError1 != nil) {
	    log.Errorf("Error in ConvertToDataDefinition for StorageClientImpl.list method's errors.NotFound error - %s",
	        bindings.VAPIerrorsToError(errError1).Error())
        return nil
	  }
	  errorDefinitions = append(errorDefinitions, errDef1.(data.ErrorDefinition))
