...
type ItemClient interface {
    ...
    // @throws NotFound  if the library item with ``source_library_item_id`` does not exist, or if the library referenced by the library.ItemModel#libraryId property of ``destination_create_spec`` does not exist.
    ...
    // @throws InvalidElementType  if the library.ItemModel#libraryId property of ``destination_create_spec`` refers to a subscribed library.
    ...
    // @throws NotAllowedInCurrentState  if the content of the source library item specified by ``source_library_item_id``, or the content of the target library specified by the library ID (see library.ItemModel#libraryId) property of ``destination_create_spec`` has been deleted from the storage backings (see null) associated with it.
    ...
    // * The resource ``com.vmware.content.Library`` referenced by the property library.ItemModel#libraryId requires ``ContentLibrary.AddLibraryItem``.
    Copy(clientTokenParam *string, sourceLibraryItemIdParam string, destinationCreateSpecParam library.ItemModel) (string, error) 
    ...
    // The return value will be an identifier for the resource type: ``com.vmware.content.library.Item``.
    // @throws NotFound  if the library.ItemModel#libraryId property of ``create_spec`` refers to a library that does not exist.
    // @throws InvalidArgument  if one of the following is true for the new library item: 
    //
    // * name is empty
    // * name exceeds 80 characters
    // * description exceeds 2000 characters
    // @throws InvalidArgument  if the ``client_token`` does not conform to the UUID format.
    // @throws InvalidElementType  if the library.ItemModel#libraryId property of ``create_spec`` refers to a subscribed library.
    // @throws NotAllowedInCurrentState  if the content of the library specified by the library ID (see library.ItemModel#libraryId) property of ``create_spec`` has been deleted from the storage backings (see null) associated with it.
    // @throws AlreadyExists  if there is already a library item with same name in the library.
    // @throws com.vmware.vapi.std.errors.Unauthorized if you do not have all of the privileges described as follows: 
    //
    // * The resource ``com.vmware.content.Library`` referenced by the property library.ItemModel#libraryId requires ``ContentLibrary.AddLibraryItem``.
    Create(clientTokenParam *string, createSpecParam library.ItemModel) (string, error) 
    ...
    // Returns the library.ItemModel with the given identifier.
    //
    // @param libraryItemIdParam  Identifier of the library item to return.
    // The parameter must be an identifier for the resource type: ``com.vmware.content.library.Item``.
    // @return The library.ItemModel instance with the given ``library_item_id``.
    ...
    Get(libraryItemIdParam string) (library.ItemModel, error) 
    ...
    // Returns identifiers of all the visible (as determined by authorization policy) library items matching the requested FindSpec.
    //
    ...
    // * The resource ``com.vmware.content.Library`` referenced by the property FindSpec#libraryId requires ``System.Read``.
    ...
    // @throws NotAllowedInCurrentState  if the library item belongs to a published library with JSON persistence enabled (see library.PublishInfo#persistJsonEnabled) and the content of the library item specified by ``library_item_id`` has been deleted from the storage backings (see null) associated with it.
    // @throws AlreadyExists  if there is already a library item with same name in the library.
    // @throws com.vmware.vapi.std.errors.Unauthorized if you do not have all of the privileges described as follows: 
    //
    ...
    Update(libraryItemIdParam string, updateSpecParam library.ItemModel) error 
    ...
    Publish(libraryItemIdParam string, forceSyncContentParam bool, subscriptionsParam []DestinationSpec) error 
