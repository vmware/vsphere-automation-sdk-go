...
func (cIface *ConfigurationClientImpl) Update(modelParam content.ConfigurationModel) error {
...
func (cIface *ConfigurationClientImpl) Get() (content.ConfigurationModel, error) {
	typeConverter := cIface.connector.TypeConverter()
	...
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput content.ConfigurationModel
    ...
    if methodResult.IsSuccess() {
	    ...
	    return output.(content.ConfigurationModel), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
