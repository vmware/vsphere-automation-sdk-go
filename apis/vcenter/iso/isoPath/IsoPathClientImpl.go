...
func (iIface *IsoPathClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(iIface.interfaceName)
}

func (iIface *IsoPathClientImpl) Definition() core.InterfaceDefinition {
    return iIface.interfaceDefinition
}

func (iIface *IsoPathClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (iIface *IsoPathClientImpl) GetDatastorePath(libraryItemIdParam string) (string, error) {
	...
	sv := bindings.NewStructValueBuilder(getDatastorePathInputType(), typeConverter)
	...
	operationRestMetaData := getDatastorePathRestMetadata()
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), getDatastorePathOutputType())
...
func (iIface *IsoPathClientImpl) getDatastorePathMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(getDatastorePathInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(getDatastorePathOutputType())
      if(inputError != nil) {
