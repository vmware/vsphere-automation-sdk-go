...
func (cIface *CustomizationSpecsClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(cIface.interfaceName)
}

func (cIface *CustomizationSpecsClientImpl) Definition() core.InterfaceDefinition {
    return cIface.interfaceDefinition
}

func (cIface *CustomizationSpecsClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (cIface *CustomizationSpecsClientImpl) List(filterParam *FilterSpec) ([]Summary, error) {
...
func (cIface *CustomizationSpecsClientImpl) Create(specParam CreateSpec) (string, error) {
	typeConverter := cIface.connector.TypeConverter()
...
func (cIface *CustomizationSpecsClientImpl) Get(nameParam string) (Info, error) {
	typeConverter := cIface.connector.TypeConverter()
...
func (cIface *CustomizationSpecsClientImpl) Set(nameParam string, specParam Spec) error {
	typeConverter := cIface.connector.TypeConverter()
...
func (cIface *CustomizationSpecsClientImpl) Export(nameParam string, formatParam Format) (string, error) {
	typeConverter := cIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(cIface.interfaceIdentifier, "export")
	sv := bindings.NewStructValueBuilder(exportInputType(), typeConverter)
	sv.AddStructField("Name", nameParam)
	...
	operationRestMetaData := exportRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= cIface.Invoke(cIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput string
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), exportOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(string), nil
...
func (cIface *CustomizationSpecsClientImpl) ImportSpecification(customizationSpecParam string) (CreateSpec, error) {
	typeConverter := cIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(cIface.interfaceIdentifier, "import_specification")
	sv := bindings.NewStructValueBuilder(importSpecificationInputType(), typeConverter)
	sv.AddStructField("CustomizationSpec", customizationSpecParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput CreateSpec
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := importSpecificationRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= cIface.Invoke(cIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput CreateSpec
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), importSpecificationOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(CreateSpec), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInError)
...
func (cIface *CustomizationSpecsClientImpl) exportMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(exportInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(exportOutputType())
      if(inputError != nil) {
...
func (cIface *CustomizationSpecsClientImpl) importSpecificationMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(importSpecificationInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(importSpecificationOutputType())
      if(inputError != nil) {
