...
func (cIface *ClustersClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(cIface.interfaceName)
}

func (cIface *ClustersClientImpl) Definition() core.InterfaceDefinition {
    return cIface.interfaceDefinition
}

func (cIface *ClustersClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (cIface *ClustersClientImpl) Upgrade(clusterParam string, specParam UpgradeSpec) error {
...
func (cIface *ClustersClientImpl) UpgradeMultiple(specsParam map[string]UpgradeSpec) (map[string]Result, error) {
	typeConverter := cIface.connector.TypeConverter()
	methodIdentifier := core.NewMethodIdentifier(cIface.interfaceIdentifier, "upgrade_multiple")
	sv := bindings.NewStructValueBuilder(upgradeMultipleInputType(), typeConverter)
	sv.AddStructField("Specs", specsParam)
	inputDataValue, inputError := sv.GetStructValue()
	if inputError != nil {
        var emptyOutput map[string]Result
		return emptyOutput, bindings.VAPIerrorsToError(inputError)
	}
	operationRestMetaData := upgradeMultipleRestMetadata()
	connectionMetadata := map[string]interface{}{lib.REST_METADATA: operationRestMetaData}
	cIface.connector.SetConnectionMetadata(connectionMetadata)
	methodResult:= cIface.Invoke(cIface.connector.NewExecutionContext(), methodIdentifier, inputDataValue)
	var emptyOutput map[string]Result
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), upgradeMultipleOutputType())
		if errorInOutput != nil {
		    return emptyOutput, bindings.VAPIerrorsToError(errorInOutput)
		}
	    return output.(map[string]Result), nil
	} else {
		methodError, errorInError := typeConverter.ConvertToGolang(methodResult.Error(), cIface.errorBindingMap[methodResult.Error().Name()])
		if errorInError != nil {
...
func (cIface *ClustersClientImpl) Get(clusterParam string) (Info, error) {
	typeConverter := cIface.connector.TypeConverter()
...
func (cIface *ClustersClientImpl) List() ([]Summary, error) {
	typeConverter := cIface.connector.TypeConverter()
...
func (cIface *ClustersClientImpl) upgradeMultipleMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(upgradeMultipleInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(upgradeMultipleOutputType())
      if(inputError != nil) {
