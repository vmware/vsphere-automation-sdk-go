...
func (bIface *BaseImagesClientImpl) Identifier() core.InterfaceIdentifier {
      return core.NewInterfaceIdentifier(bIface.interfaceName)
}

func (bIface *BaseImagesClientImpl) Definition() core.InterfaceDefinition {
    return bIface.interfaceDefinition
}

func (bIface *BaseImagesClientImpl) MethodDefinition(mi core.MethodIdentifier) *core.MethodDefinition {
...
func (bIface *BaseImagesClientImpl) ImportFromImgdb(imgdbParam []byte) (string, error) {
	...
	sv := bindings.NewStructValueBuilder(importFromImgdbInputType(), typeConverter)
	...
	operationRestMetaData := importFromImgdbRestMetadata()
    ...
    if methodResult.IsSuccess() {
		output, errorInOutput := typeConverter.ConvertToGolang(methodResult.Output(), importFromImgdbOutputType())
...
func (bIface *BaseImagesClientImpl) List() ([]Summary, error) {
	typeConverter := bIface.connector.TypeConverter()
...
func (bIface *BaseImagesClientImpl) Get(versionParam string) (Info, error) {
	typeConverter := bIface.connector.TypeConverter()
...
func (bIface *BaseImagesClientImpl) importFromImgdbMethodDefinition() *core.MethodDefinition {
      ...
      input, inputError := typeConverter.ConvertToDataDefinition(importFromImgdbInputType())
      output, outputError := typeConverter.ConvertToDataDefinition(importFromImgdbOutputType())
      if(inputError != nil) {
